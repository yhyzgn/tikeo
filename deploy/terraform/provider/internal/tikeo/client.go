...
func NewClient(config Config) (*Client, error) {
	...
	if endpoint == "" {
		return nil, fmt.Errorf("tikeo endpoint is required")
	}
	parsed, err := url.Parse(endpoint)
	if err != nil || parsed.Scheme == "" || parsed.Host == "" {
		return nil, fmt.Errorf("tikeo endpoint must be an absolute URL")
	}
	if strings.TrimSpace(config.APIToken) == "" {
		return nil, fmt.Errorf("tikeo api token is required")
	}
...
func (c *Client) do(ctx context.Context, method, path string, body []byte) (json.RawMessage, error) {
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return nil, fmt.Errorf("tikeo API %s %s returned %s: %s", method, path, response.Status, string(payload))
	}
	var envelope struct {
		Code    int             `json:"code"`
	...
	if err := json.Unmarshal(payload, &envelope); err != nil {
		return nil, fmt.Errorf("tikeo API returned invalid JSON envelope: %w", err)
	}
	if envelope.Code != 0 {
		return nil, fmt.Errorf("tikeo API returned code %d: %s", envelope.Code, envelope.Message)
	}
