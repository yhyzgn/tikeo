...
func TestClientExportsManifestAndPostsDiffWithAuthorization(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case ManifestPath:
			...
			_, _ = w.Write([]byte(`{"code":0,"data":{"manifest":{"apiVersion":"tikee.io/v1alpha1","kind":"TikeeManifest","scope":{},"resources":[]},"manifestYaml":"apiVersion: tikee.io/v1alpha1","checksum":"sha256:abc"}}`))
		case DiffPath:
	...
	if _, err := client.DiffManifest(context.Background(), json.RawMessage(`{"apiVersion":"tikee.io/v1alpha1","kind":"TikeeManifest","scope":{},"resources":[]}`)); err != nil { t.Fatal(err) }
...
func TestClientFailsClosedOnInvalidConfigAndManifest(t *testing.T) {
	...
	client, err := NewClient(Config{Endpoint: "https://tikee.example", APIToken: "token"})
	if err != nil { t.Fatal(err) }
