...
REPORT_DIR="${TIKEO_SCRIPT_REPORT_DIR:-$TIKEO_SMOKE_REPORT_DIR}"
RUN_ID="${TIKEO_SCRIPT_RUN_ID:-script-$(date -u +%Y%m%dT%H%M%SZ)-$$}"
mkdir -p "$REPORT_DIR"
...
PY
  ...
  tikeo_smoke_api "$API_URL" POST /api/v1/scripts "$body" > "$file"
...
PY
  tikeo_smoke_record_case "script-$language-create" passed "$file" "created $language script definition"
}
...
create_script shell 'echo tikeo-shell-smoke'
create_script python 'print("tikeo-python-smoke")'
create_script javascript 'console.log("tikeo-js-smoke")'
create_script typescript 'console.log("tikeo-ts-smoke")'
create_script rhai 'print("tikeo-rhai-smoke");'
