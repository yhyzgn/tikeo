...
做迁移评估时，可以用 `tikeo migrate` 读取 XXL-JOB 或 PowerJob 的 JSON 导出并生成 dry-run 迁移报告：
...
该工具会输出 Tikeo job 草案和 unsupported-feature 警告。它刻意保持 report-only：不连接旧系统数据库，也不会自动创建 Tikeo Jobs。详见[从 XXL-JOB 或 PowerJob 迁移](https://docs.tikeo.net/zh-CN/docs/integrations/migrating-from-legacy-schedulers)。
