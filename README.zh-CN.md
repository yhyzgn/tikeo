...
| `structured.sdkProcessors` | `TIKEO_WORKER_SDK_PROCESSORS` | 否 | 随 demo 而定 | 对外声明可派发的 SDK processor 名称。 |
...
配置加载顺序是默认值、YAML/TOML 配置文件、环境变量覆盖。Docker/Compose 中，Tikeo 服务配置统一放在 `config/tikeo.yml`；`.env` 只放镜像 tag、宿主机端口、volume 名、数据库容器凭据、时区和容器内存策略。
