...
- **旧调度器迁移 CLI**：无副作用的 `tikeo-migrate plan`、本地 `apply` 到隔离 Worker 副本、已复核 bundle、预发手动导入和 release assets。
...
| 4. 代码改造 | 在隔离副本中添加 Tikeo Worker 依赖、就地 Spring 配置占位符、processor 注解或适配器。 | `tikeo-migrate apply --bundle ./.tikeo-migration --output-project ../legacy-worker-tikeo` + 编译/测试。 | 原项目未被修改；迁移副本可编译，原旧调度器配置文件中已包含 Tikeo 占位配置，processor name 与 job 草案匹配。 |
...
生成的迁移包刻意保守：`plan` 不改旧源码、不写 Tikeo 数据；它可能用只读连接读取旧调度器数据库来增强复核包。本地 `apply` 默认写入单独的输出项目，生成 `code-apply-evidence.json` 与 `CODE_MIGRATION_REPORT.md`，并把完整的 `tikeo.worker.*` / `tikeo.management.*` 占位配置追加到原来已经包含 XXL-JOB 或 PowerJob 配置的 `application*` / `bootstrap*` 文件中。它不会创建单独的迁移 profile，也不会调用 Tikeo Server；导入已复核任务时，在补全部署配置后手动执行。完整说明见[旧调度器迁移指南](https://docs.tikeo.net/zh-CN/docs/integrations/migrating-from-legacy-schedulers)。
