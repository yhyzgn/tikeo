...
Tikeo 提供独立的 `tikeo-migrate` CLI 做迁移评估。它会读取 XXL-JOB 或 PowerJob 的 JSON 导出、扫描 Java/Spring 旧项目，并生成非破坏性的迁移包。推荐采用“约定优先”的方式：把旧调度器导出的 JSON 放在旧 Worker 项目根目录，然后在该目录执行工具；只有目录或文件名不规范时才手动传参数覆盖。

```bash
cd ./legacy-worker
# 自动探测 ./xxl-job-export.json 或 ./powerjob-export.json、当前 Java 项目，并写入 ./.tikeo-migration
tikeo-migrate plan

# 复核迁移包后，先 dry-run API 写入。--bundle 也默认读取 ./.tikeo-migration。
tikeo-migrate apply-data --endpoint http://127.0.0.1:9090 --api-key "$TIKEO_MIGRATION_API_KEY" --dry-run
```

非标准目录仍可使用覆盖参数，例如 `--from xxl-job`、`--input ./exports/jobs.json`、`--project ./legacy-worker`、`--output-dir ./migration-bundle`、`--namespace ops`、`--app billing`。该工具会输出 Tikeo job 草案、Java 依赖/处理器补丁、unsupported-feature 警告和验收清单。默认是非破坏性的：`plan` 不改旧项目、不写 Tikeo 数据；真正创建 Job 的动作隔离在显式 `apply-data` 命令中。详见[从 XXL-JOB 或 PowerJob 迁移](https://docs.tikeo.net/zh-CN/docs/integrations/migrating-from-legacy-schedulers)。

Release 会提供可直接运行的 `tikeo-migrate` 压缩包，覆盖 Linux、macOS Intel、macOS Apple Silicon 和 Windows。用户只需要从 GitHub Release 下载匹配的 `tikeo-migrate-${TIKEO_VERSION}-<target>.tar.gz` 或 `.zip`，解压后把二进制放进旧项目目录，或放到 `PATH` 即可。
...
flowchart TD
  ...
  D --> E[复核 .tikeo-migration 报告和 Java 补丁建议]
  E --> F[在分支上改代码并在 staging 启动 Tikeo Worker]
  F --> G[执行: tikeo-migrate apply-data --dry-run]
  G --> H[把 ready jobs 写入 staging]
  H --> I[逐个触发迁移任务并对比日志/结果]
  I --> J{行为对账通过?}
  J -- 否 --> E
  J -- 是 --> K[双跑观察，然后切流并禁用旧调度]
```
