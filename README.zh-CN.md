...
做迁移评估时，应使用独立的 `tikeo-migrate` CLI 读取 XXL-JOB 或 PowerJob 的 JSON 导出、扫描 Java/Spring 旧项目，并生成非破坏性的迁移包。推荐采用“约定优先”的方式：把旧调度器导出的 JSON 放在旧 Worker 项目根目录，然后在该目录执行工具；只有目录或文件名不规范时才手动传参数覆盖。
