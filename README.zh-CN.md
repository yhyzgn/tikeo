...
Tikeo 提供独立的 `tikeo-migrate` CLI 做迁移评估。请把它当成 **review-first 的迁移助手**，而不是盲目一键转换器：它读取 XXL-JOB 或 PowerJob 的 JSON 导出，扫描旧 Java/Spring Worker 项目，生成 Tikeo Job 草案，并输出一份可复核的迁移包；只有复核通过后才进入导入阶段。

最简单的路径是“约定优先”：

```bash
cd ./legacy-worker
# 先把 ./xxl-job-export.json 或 ./powerjob-export.json 放到这个目录。
tikeo-migrate plan
...
只有非标准目录才需要覆盖参数：`--from xxl-job`、`--input ./exports/jobs.json`、`--project ./legacy-worker`、`--output-dir ./migration-bundle`、`--namespace ops`、`--app billing`。
...
```mermaid
flowchart TD
  A[下载 tikeo-migrate] --> B[导出旧调度器 jobs JSON]
  B --> C[把 JSON 放到旧 Java Worker 项目根目录]
...
| 1. 导出 | 把旧调度器状态保存为审计输入。 | `xxl-job-export.json` 或 `powerjob-export.json`。 | 原始导出文件未被修改，并有明确路径/hash。 |
