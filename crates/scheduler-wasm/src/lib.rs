...
use scheduler_core::{WasmCapabilities, WasmProcessorSpec, WasmSpecError};
use thiserror::Error;
...
impl WasmExecutor {
    ...
    /// Execute a module with the provided scheduler WASM policy.
    ///
    ...
    fn fuel_limit_interrupts_busy_loop() {
        ...
        let module = wat(r#"(module
              (func (export "_start")
                (loop br 0)))"#);
        let spec = WasmProcessorSpec {
            resources: scheduler_core::WasmResourcePolicy {
                fuel: 10,
                ..scheduler_core::WasmResourcePolicy::default()
            },
            ..WasmProcessorSpec::default()
        };
