...
use tikeo_storage::{AppendJobInstanceLog, CreateAuditLog, CreateJobInstance};
...
async fn verify_webhook_signature(
    ...
    let secret_ref = request
        .secret_ref
        .as_deref()
        .or_else(|| header_str(headers, "x-tikeo-webhook-secret-ref"));
    let signature = request
        .signature
        .as_deref()
        .or_else(|| header_str(headers, "x-tikeo-webhook-signature"));
    let timestamp = request.timestamp.or_else(|| {
        header_str(headers, "x-tikeo-webhook-timestamp").and_then(|value| value.parse().ok())
    });
    let nonce = request
        .nonce
        .as_deref()
        .or_else(|| header_str(headers, "x-tikeo-webhook-nonce"));
