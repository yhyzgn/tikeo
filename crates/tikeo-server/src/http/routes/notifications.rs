...
pub async fn get_notification_message_trace(
    ...
    let instance = if let Some(instance_id) = instance_id.as_deref() {
        state
            .instances
            .get(instance_id)
            .await
    ...
    let job = if let Some(instance) = instance.as_ref() {
        state
            .jobs
            .get(&instance.job_id)
            .await
            .map_err(|error| ApiError::storage(&error))?
    } else if message.resource_type == "job" {
        state
            .jobs
    ...
    let logs = if let Some(instance_id) = instance_id.as_deref() {
        notification_trace_logs(&state, instance_id).await?
    } else {
    ...
    Ok(Json(ApiResponse::success(
        NotificationMessageTraceResponse {
            ...
            instance: instance.map(|item| NotificationTraceInstance {
                ...
                worker_id: item.result.map(|result| result.worker_id),
            }),
            logs,
        },
    )))
...
async fn notification_trace_logs(
    ...
    Ok(NotificationTraceLogs {
        url: Some(format!("/instances/{instance_id}/logs")),
        excerpt,
        truncated: total > 80,
    })
}
