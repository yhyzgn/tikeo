...
use tikeo_core::{ScriptExecutionPolicy, ScriptReleaseGrantSet};
use tikeo_storage::{VerifiedScriptReleaseGrants, VerifiedScriptReleaseSignature};
...
async fn enforce_release_signature_gate(
    ...
    version: &tikeo_storage::ScriptVersionSummary,
    headers: &HeaderMap,
...
async fn append_release_gate_audit(
    ...
    if let Err(error) = state
        .audit
        .append(tikeo_storage::CreateAuditLog {
...
async fn resolve_release_version(
    ...
    id: &str,
    version_number: i64,
) -> Result<tikeo_storage::ScriptVersionSummary, ApiError> {
    state
        .scripts
        .versions()
...
pub async fn list_script_versions(
    ...
    headers: HeaderMap,
    Path(id): Path<String>,
) -> Result<Json<ApiResponse<Vec<tikeo_storage::ScriptVersionSummary>>>, ApiError> {
    auth::require_permission(&headers, &state, "scripts", "read").await?;
    let versions = state
        .scripts
        .versions()
...
fn policy_diff(
    v1: &tikeo_storage::ScriptVersionSummary,
    v2: &tikeo_storage::ScriptVersionSummary,
