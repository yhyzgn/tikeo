...
use std::sync::Arc;
use tikeo_storage::{CreateOidcAuthState, OidcAuthStateRepository};
use tracing::warn;
...
fn bearer_token(headers: &HeaderMap) -> Result<String, ApiError> {
    ...
    if let Some(value) = headers.get("x-tikeo-token") {
        return value
            .to_str()
            .map(str::to_owned)
            .map_err(|_| ApiError::unauthorized("invalid x-tikeo-token header"));
    }
...
fn validate_api_token_scopes(
    scopes: Vec<String>,
    permissions: &[tikeo_storage::PermissionSummary],
...
fn validate_api_token_ttl(
    ...
    policy: &tikeo_config::ApiTokenConfig,
) -> Result<i64, ApiError> {
