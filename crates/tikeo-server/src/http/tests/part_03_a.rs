...
async fn metrics_summary_reports_storage_registry_and_alert_counts() {
    ...
    assert_eq!(json["data"]["queue"]["pendingByShardOwner"]["test-node"], 2);
    ...
    assert_eq!(json["data"]["workflows"]["shardsByStatus"]["succeeded"], 2);
    assert_eq!(
...
async fn tenant_secret_store_creates_lists_and_deletes_scoped_secret_refs() {
    ...
    let id = created["data"]["id"]
        .as_str()
        .unwrap_or_else(|| panic!("secret id should exist"))
        .to_owned();
    ...
    let list_response = app
        ...
        .oneshot(
            admin_request_builder(
                app.clone(),
                "GET",
                "/api/v1/secrets?namespace=default&app=billing",
            )
            .await,
        )
        .await
    ...
    let delete_response = app
        .clone()
        .oneshot(
            admin_request_builder(app.clone(), "DELETE", format!("/api/v1/secrets/{id}")).await,
        )
        .await
    ...
    let read_audit = app
        ...
        .oneshot(
            admin_request_builder(
                app.clone(),
                "GET",
                "/api/v1/audit-logs?action=read&resource_type=secret&page_size=1",
            )
            .await,
        )
        .await
    ...
    let create_audit = app
        ...
        .oneshot(
            admin_request_builder(
                app.clone(),
                "GET",
                "/api/v1/audit-logs?action=create&resource_type=secret&page_size=1",
            )
            .await,
        )
        .await
    ...
    let delete_audit = app
        ...
        .oneshot(
            admin_request_builder(
                app,
                "GET",
                "/api/v1/audit-logs?action=delete&resource_type=secret&page_size=1",
            )
            .await,
        )
        .await
...
async fn inbound_webhook_rejects_replayed_signed_nonce() {
    ...
    let signature = inbound_webhook_signature(
...
async fn scheduling_advice_reports_worker_capability_readiness() {
    ...
    assert_eq!(
        ...
        "normal processor 'billing.advice'"
    );
    ...
    assert_eq!(
        json["data"]["eligibleWorkers"].as_array().map(Vec::len),
        Some(1)
    );
}
...
async fn canary_metrics_gate_auto_rolls_back_failed_target_before_trigger() {
    ...
    assert_eq!(
        created_json["data"]["canaryPolicy"]["metricsGateEnabled"],
        true
    );
    ...
    assert_eq!(
        json["data"]["canaryRouting"]["metricsGate"]["status"],
        "rollback"
    );
    assert_eq!(
        json["data"]["canaryRouting"]["metricsGate"]["failedSamples"],
        2
    );
    ...
    assert_eq!(
        updated_main.canary_job_id.as_deref(),
        Some(canary.id.as_str())
    );
    ...
    let audit_json: Value = serde_json::from_slice(&body)
        .unwrap_or_else(|error| panic!("audit body should be JSON: {error}"));
    assert_eq!(
        audit_json["data"]["items"][0]["action"],
