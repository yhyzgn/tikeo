...
use tikeo_storage::{
    ...
    JobInstanceRepository, NotificationDeliveryAttemptFilters, NotificationMessageFilters,
    connect_and_migrate,
};
...
async fn job_instance_event_materializes_message_and_delivery_attempts() {
    ...
    let center =
        NotificationCenter::new(channels, policies, messages.clone(), attempts.clone(), jobs);
    let emitted = center
        .emit_job_instance_event(&instance, JobNotificationEvent::Failed, Some("exit 2"))
        .await
        .unwrap_or_else(|error| panic!("notification should emit: {error}"));
