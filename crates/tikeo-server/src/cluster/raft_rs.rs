...
//! ticker. Tikeo ownership is granted only after raft-rs observes real leadership
...
pub struct RaftRsBootstrapStatus {
    /// Tikeo string node id from config.
    pub node_id: String,
    ...
    /// Initial raft-rs role after construction; must not be treated as tikeo ownership.
        ...
        for message in messages {
            ...
            tokio::spawn(async move {
                ...
                if let Some(token) = token {
                    builder = builder.header("x-tikeo-raft-token", token);
                }
...
impl RaftRuntimeCoordinator {
    ...
    ) -> Result<SharedClusterCoordinator, tikeo_storage::DbErr> {
...
> {
    ...
    // A process restart must not inherit stale tikeo authority. The next observed
...
async fn restore_persisted_storage(
...
) -> Result<usize, tikeo_storage::DbErr> {
    ...
    if !entries.is_empty() {
        node.raft
            ...
            .append(&entries)
            .map_err(|error| {
                tikeo_storage::DbErr::Custom(format!(
...
fn stored_log_entry_to_raft(row: &RaftLogEntrySummary) -> Result<Entry, tikeo_storage::DbErr> {
    ...
    let data = STANDARD.decode(&row.data).map_err(|error| {
        tikeo_storage::DbErr::Custom(format!("raft log entry data base64 invalid: {error}"))
    })?;
    entry.set_data(data.into());
    if let Some(context) = row.context.as_deref() {
        let context = STANDARD.decode(context).map_err(|error| {
            tikeo_storage::DbErr::Custom(format!("raft log entry context base64 invalid: {error}"))
        })?;
...
fn stored_entry_type(entry_type: &str) -> Result<EntryType, tikeo_storage::DbErr> {
    match entry_type {
        ...
        other => Err(tikeo_storage::DbErr::Custom(format!(
            "unsupported persisted raft entry type: {other}"
        ))),
    }
}

fn decode_conf_state(conf_state: &str) -> Result<ConfState, tikeo_storage::DbErr> {
    let bytes = STANDARD.decode(conf_state).map_err(|error| {
        tikeo_storage::DbErr::Custom(format!("raft conf_state base64 invalid: {error}"))
    })?;
    let mut decoded = ConfState::new();
    decoded.merge_from_bytes(&bytes).map_err(|error| {
        tikeo_storage::DbErr::Custom(format!("raft conf_state protobuf invalid: {error}"))
    })?;
    Ok(decoded)
}
...
async fn process_ready(
    ...
    node.raft
        ...
        .map_err(|error| {
            tikeo_storage::DbErr::Custom(format!("raft-rs MemStorage append failed: {error}"))
        })?;
...
async fn apply_committed_entries(
    ...
    entries: &[Entry],
) -> Result<Option<u64>, tikeo_storage::DbErr> {
...
fn parse_entry_command(entry: &Entry) -> RaftCommandApply {
    ...
    match serde_json::from_slice::<TikeoRaftCommand>(entry.get_data()) {
        Ok(command) => parse_tikeo_command(command, entry.get_index()),
...
fn parse_tikeo_command(command: TikeoRaftCommand, log_index: u64) -> RaftCommandApply {
