...
pub type ServiceAccountApiResponse = ApiResponse<tikee_storage::ServiceAccountSummary>;
pub type ServiceAccountListApiResponse = ApiResponse<Vec<tikee_storage::ServiceAccountSummary>>;
...
pub async fn list_service_accounts(
    ...
    let items = tikee_storage::ServiceAccountRepository::new(state.users.db())
        .list()
        .await
...
pub async fn create_service_account(
    ...
    let created = tikee_storage::ServiceAccountRepository::new(state.users.db())
        .create(tikee_storage::CreateServiceAccount {
            name: request.name,
...
pub async fn update_service_account(
    ...
    let Some(updated) = tikee_storage::ServiceAccountRepository::new(state.users.db())
        .update(
            &id,
            tikee_storage::UpdateServiceAccount {
                name: request.name,
                description: request.description,
                namespace: request.namespace,
                app: request.app,
    ...
    let api_keys = tikee_storage::SdkApiKeyRepository::new(state.users.db());
...
pub async fn disable_service_account(
    ...
    let principal = auth::require_permission(&headers, &state, "tenants", "manage").await?;
    let repo = tikee_storage::ServiceAccountRepository::new(state.users.db());
    let disabled = repo
    ...
    tikee_storage::SdkApiKeyRepository::new(state.users.db())
        .revoke_keys_for_service_account(&id, &principal.username)
        .await
        .map_err(|error| ApiError::storage(&error))?;
    audit(
