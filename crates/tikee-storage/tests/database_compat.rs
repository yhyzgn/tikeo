//! Cross-database compatibility smoke tests for tikee storage.
...
async fn external_database_compatibility_smoke() {
    ...
    if urls.is_empty() {
        eprintln!(
            "skip external database compatibility smoke: set TIKEE_TEST_DATABASE_URLS, \
             TIKEE_TEST_POSTGRES_URL, or TIKEE_TEST_MYSQL_URL"
        );
        return;
    }
...
async fn run_storage_smoke(database_url: &str) {
    ...
    let plugin = plugin_repository
        .create_plugin(CreatePlugin {
            ...
            processor_types: vec![PluginProcessorTypeSummary {
                ...
                artifact_ref: Some("oci://example.invalid/tikee/compat-plugin:1".to_owned()),
                container_image: Some("example.invalid/tikee/compat-plugin:1".to_owned()),
    ...
    let script = script_repository
        .create_script(CreateScript {
            ...
            allow_network: false,
            allowed_env_vars: Some(serde_json::json!(["TIKEE_COMPAT"]).to_string()),
            policy_json: Some(
    ...
    assert_eq!(
        ...
        Some(["TIKEE_COMPAT".to_owned()].as_slice())
    );
...
fn external_database_urls() -> Vec<String> {
    ...
    if let Ok(value) = std::env::var("TIKEE_TEST_DATABASE_URLS") {
        urls.extend(
            value
                .split(',')
                .map(str::trim)
    ...
    for name in ["TIKEE_TEST_POSTGRES_URL", "TIKEE_TEST_MYSQL_URL"] {
