...
2. **脚本版本化与签名**：脚本内容按 content hash 存储；生产环境脚本必须经过审批、签名或可信发布流水线。
...
9. **审计与可追溯**：记录脚本版本、提交人、审批人、执行 Worker、输入摘要、能力清单、资源用量、网络目标和 artifact hash。
...
| Scripts | `GET/POST /api/v1/scripts`、`POST /api/v1/scripts/{script}:publish`、`:approve`、`:rollback` | 动态脚本版本、发布、审批、回滚 |
...
| Scripts | 脚本列表、在线编辑、版本、审批、沙箱策略、执行记录 | 多语言动态脚本治理 |
