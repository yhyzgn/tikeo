...
- **Dynamic membership design gate**：成员变更拆成两层：1) 普通 `EntryNormal` 的 `raft_member_upsert` 只维护平台可观测成员目录，允许 `configured/joining/active/leaving/removed` 等软状态；2) 真正改变 raft voters/learners 必须由单独的 membership proposal API 创建变更意图，Leader 校验当前 term、节点可达性、传输 token、quorum 风险后调用 raft-rs `propose_conf_change`，并且只有 committed `EntryConfChange/EntryConfChangeV2` 被显式解码、持久化、应用到 raft storage `ConfState` 后，才能把成员目录推进到 `active/removed`。在该流程落地前，所有 config-change entry 都保持 gate，防止未设计完整的动态 membership 破坏 quorum。
...
- [ ] Server 集群 (Raft 共识；Phase2 已完成安全基础，已改用 TiKV raft-rs 并完成 bootstrap/ticker/inbound inbox/outbound skeleton，apply/leader fencing 继续推进)
  ...
  - [ ] raft-rs dynamic membership proposal API + committed ConfChange/ConfChangeV2 解码应用
