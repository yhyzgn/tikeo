...
tikee 是一个用 Rust 从零构建的分布式任务调度与计算平台，目标是**完全覆盖 PowerJob 的全部功能特性**，同时在性能、易用性、部署体验和安全模型上实现质的飞跃。

### 1.2 为什么要从 0 开发

经过 xxl-job 与 PowerJob 源码调研后，结论明确：**xxl-job 是能力上限不足，PowerJob 是能力堆叠后架构债过重**。二者都不适合作为企业平台统一调度底座。

| 痛点 | xxl-job 现状 | PowerJob 现状 | tikee 目标 |
|------|--------------|---------------|----------------|
...
因此 tikee 不是“PowerJob 的 Rust 版本”，也不是“xxl-job 加功能”。它是面向企业平台公共调度服务的一次重新建模：**用更少的核心抽象承载更多、更可靠、更安全的能力**。
### 1.3 源码调研后的核心结论

本设计不再只以 PowerJob 为单一参照，而是把 xxl-job 与 PowerJob 都作为反例基线：

| 结论 | xxl-job 源码表现 | PowerJob 源码表现 | tikee 设计取舍 |
|------|------------------|-------------------|--------------------|
...
因此，tikee 的目标不是“复刻 PowerJob”，而是保留其有价值的功能模型（多调度方式、MapReduce、DAG、官方处理器），同时替换掉不适合云原生公共服务的通信、选举、状态、安全和可观测性设计。
关键源码依据（用于支撑后文设计取舍）：

- xxl-job 调度类型与 DB 锁：`xxl-job-admin/src/main/java/com/xxl/job/admin/tikee/type/ScheduleTypeEnum.java`、`xxl-job-admin/src/main/resources/mapper/XxlJobLockMapper.xml`、`JobScheduleHelper.java`
- xxl-job Executor 反向调用与 child_jobid：`xxl-job-core/src/main/java/com/xxl/job/core/executor/XxlJobExecutor.java`、`xxl-job-admin/src/main/java/com/xxl/job/admin/tikee/complete/JobCompleter.java`
...
3. **Single Binary** — 编译产物为一个可执行文件，`./tikee serve` 即启动
...
| 功能 | xxl-job | PowerJob | tikee | 增强重点 |
|------|---------|----------|--------------|----------|
...
| 功能 | xxl-job | PowerJob | tikee | 安全/体验增强 |
|------|---------|----------|--------------|----------------|
...
```mermaid
graph TB
    subgraph Cluster["tikee Cluster"]
        direction TB
...
```rust
// tikee Rust SDK — 3 行代码集成
use tikee_sdk::prelude::*;
...
```python
# tikee Python SDK
from tikee import Worker, TaskContext, TaskResult

worker = Worker("tikee.example.com:9090", app_name="my-service")
...
import (
    tikee "github.com/tikee/sdk-go"
)

func main() {
    w := tikee.NewWorker("tikee.example.com:9090",
        tikee.WithAppName("my-service"))

    w.Register("my_task", func(ctx tikee.TaskContext) tikee.TaskResult {
        return tikee.Success("done")
    })
...
对比 PowerJob 的集成方式——需要添加 Maven 依赖、配置 properties、实现 Java 接口、Spring Boot 启动——tikee 的多语言 SDK 将集成成本降低到**任意语言 3-5 行代码**。

#### 4.2.3 Java Spring Boot Starter SDK

Java 端 SDK 优先支持 Spring Boot Starter 模式，目标是让现有 Spring Boot 业务以最小改造接入 tikee Worker Tunnel。
...
public class BillingTasks {
    @TikeeProcessor("billing.reconcile")
...
```yaml
tikee:
  server: https://tikee.example.com
...
- 与 Server 的 Worker Tunnel 主动连接、注册、心跳、状态上报、日志上报和日志订阅。当前已完成真实 gRPC 连接、注册、心跳、日志、任务结果回传，并已支持将 `@TikeeProcessor` 方法适配为真实任务处理器（通过 `DispatchTask.processor_name` 匹配 processor name，payload 支持 UTF-8 String / byte[] / TaskContext；空值兼容回退到 `job_id`）。
...
- 默认不暴露入站端口，不要求业务 Service 被 tikee 访问。

#### 4.2.4 动态脚本处理器设计

动态脚本是一等处理器能力，用于低频运维任务、轻量数据处理、迁移脚本、Webhook 编排和临时自动化。设计目标是**多语言可用，但默认不信任脚本**：脚本永远不在 tikee Server 进程内执行，只能在 Worker 侧的受控执行环境中运行。

**支持语言分层**：

| 级别 | 语言/运行时 | 适用场景 | 安全策略 |
|------|-------------|----------|----------|
| 默认支持 | Shell、Python、Node.js/TypeScript、PowerShell | 运维脚本、数据处理、API 编排 | **WASM/Wasmtime 通用脚本沙箱为默认后端**，默认无网络、无宿主文件系统、资源限额；Worker 运维可显式切换 Docker/Podman/custom 后端 |
...
3. **最小权限 capability**：脚本声明所需能力，例如 `network.egress`、`fs.read:/data/input`、`secret:db-readonly`；未声明能力默认不可用。Worker 注册时必须声明**语言能力**：普通脚本使用 `script:<language>`（例如 `script:shell`、`script:python`、`script:node`、`script:powershell`、`script:rhai`），直接 WASM 模块使用 `script:wasm`。能力不等同于后端类型：`script:shell` 默认可由 Wasmtime/WASM 通用沙箱后端实现，也可由运维显式切换到 Docker、Podman 或自定义后端；仅受控 Worker Pool 可使用 `script:*` 或 `*` 作为显式兜底能力。
...
Job Definition
  ...
  -> 默认 Runner 使用 Wasmtime/WASM 通用沙箱；可按 Worker 配置切换 Docker/Podman/custom 后端
...
```bash
# 服务管理
tikee serve                           # 启动 server（单机模式，SQLite）
tikee serve --cluster --db postgres   # 启动 server（集群模式）

# 任务管理
tikee job create --file job.yaml      # 从 YAML 创建任务
tikee job list --app my-service       # 列出任务
tikee job trigger <job-id>            # 手动触发
tikee job cancel <instance-id>        # 取消执行

# 工作流管理
tikee workflow create --file flow.yaml
tikee workflow trigger <wf-id>
tikee workflow visualize <wf-id>      # 终端 ASCII 可视化

# 集群管理
tikee cluster status                  # 集群健康状态
tikee cluster workers                 # Worker 列表
tikee cluster reschedule --app xxx    # 强制重新调度

# 数据管理
tikee migrate                         # 执行数据库迁移
tikee export --format json            # 导出任务定义
tikee import --file backup.json       # 导入任务定义
```
...
- tikee：Worker 主动建立 gRPC/HTTP2 长连接隧道，Server 不要求直连 Worker Pod，也不依赖 Worker 可被公网、跨 namespace 或跨集群访问。

| 场景 | xxl-job | PowerJob | tikee |
|------|---------|----------|-----------|
...
| K8s/Docker/NAT/多级网关适配 | 需要 Executor 入站可达 | 需要 Worker 入站可达，多协议多端口 | 只需要 Worker 出站访问 tikee tunnel endpoint，支持跨 namespace/cluster/VPC |
| 管理 API | HTTP MVC | HTTP REST | REST + gRPC Gateway + gRPC reflection |

因此 tikee 使用**单一 gRPC 协议**解决 Worker 通信、任务分发、日志流、状态上报和集群内部 RPC；REST 只作为管理面 API，不进入核心执行链路。
...
PowerJob 需要 4 个端口 (7700, 10086, 10010, 10077)。tikee 只需**1 个端口**：
...
tikee 的 Worker Tunnel 是对 xxl-job / PowerJob 反向调用模型的直接修正，也是跨容器、跨 namespace、跨集群部署的核心能力。Worker 注册、心跳、任务分发、取消、日志、证书轮换和配置下发都复用同一条由 Worker 主动发起的长连接。
...
2. **穿透多级网络层级**：只要求 Worker 能出站访问 tikee tunnel endpoint；中间可以是 Docker bridge、K8s Service、Ingress、API Gateway、Service Mesh、NAT Gateway、VPN、专线或跨云负载均衡。
3. **反向调用走既有通道**：Server 对 Worker 的 DispatchTask、CancelTask、Drain、RotateCert 等“反向调用”不是新建到 Worker 的连接，而是写回 Worker 已建立的双向流。
4. **跨 namespace/cluster/VPC 简化**：Worker 所在网络无需被 tikee 路由可达；注册时上报 app、namespace、cluster、region、tenant、capabilities 和 labels，Server 按逻辑属性寻址。
...
```mermaid
sequenceDiagram
    participant CRON as Tikee<br/>(CRON Engine)
...
| 数据库 | 场景 | 状态 | 说明 |
|--------|------|------|------|
| **SQLite** | 单机/开发/嵌入式 | ✅ 必须支持 | 零配置，单文件部署，`./tikee serve` 直接可用 |
...
flowchart TD
    ...
    subgraph Repo["Repository 抽象层<br/>(tikee 自定义)"]
...
tikee 的部署目标是**容器优先、网络边界无关**。Server 和 Worker 必须可以部署在不同容器、不同 Docker 网络、不同 K8s namespace、不同 K8s 集群、不同 VPC/机房甚至不同云厂商中。平台不得要求 Worker 暴露入站端口；所有注册、心跳和反向调度指令都必须通过 Worker 主动建立的 tunnel 完成。
...
5. **单入口最小暴露**：默认只暴露 tikee server 的 443/9090 tunnel/API 入口；业务命名空间默认不创建 Worker 入站 Service。

### 8.1 单机模式 (Standalone)

**零配置，开箱即用**——这是 tikee 与 PowerJob 最大的用户体验差异。

```bash
# 下载单文件 (约 15MB，含前端)
curl -LO https://github.com/tikee/tikee/releases/latest/download/tikee-linux-amd64
chmod +x tikee-linux-amd64

# 启动 (自动创建 SQLite 数据库)
./tikee-linux-amd64 serve
...
Server 与 Worker 可以在同一个 Docker 网络内，也可以位于不同 Docker host。Worker 只需要配置 `TIKEE_SERVER` 指向可出站访问的 server tunnel endpoint。
...
docker run -d \
  --name tikee \
  -p 9090:9090 \
  -v tikee-data:/var/lib/tikee \
  ghcr.io/tikee/tikee:latest

# 使用 MySQL
docker run -d \
  --name tikee \
  -p 9090:9090 \
  -e TIKEE_DB_URL="mysql://user:pass@mysql:3306/tikee" \
  ghcr.io/tikee/tikee:latest
...
docker run -d \
  --name tikee \
  -p 9090:9090 \
  -e TIKEE_DB_URL="postgres://user:pass@pg:5432/tikee" \
  ghcr.io/tikee/tikee:latest

# Worker 独立容器：不暴露端口，只主动连 server
docker run -d \
  --name tikee-worker \
  -e TIKEE_SERVER="https://tikee.example.com" \
  -e TIKEE_APP_NAME="billing-worker" \
  -e TIKEE_WORKER_POOL="prod-cn" \
  ghcr.io/tikee/tikee-worker:latest
```
...
- **Raft transport token**：`cluster.transport_token` 为可选 server-to-server shared token，应通过 Docker/K8s Secret 或环境变量 `TIKEE__CLUSTER__TRANSPORT_TOKEN` 注入，禁止提交生产 token。HTTP route 仍支持管理端 Bearer/RBAC；携带匹配 `x-tikee-raft-token` 时可绕过人工 session，用于集群内部 Raft 消息。
...
- **Container-first networking**：Raft 节点间通信必须可穿透 Docker bridge / K8s Service / LB，不能依赖 host network。`scripts/raft-bridge-e2e.sh` 会构建 alpine runtime 镜像、创建 Docker bridge 网络、用 container DNS (`tikee-0:9090` 等) 写入 peers，并通过内部 `x-tikee-raft-token` smoke-check `/healthz`、`/api/v1/cluster`、`/api/v1/cluster/diagnostics` 与 `/api/v1/raft/append-entries`；脚本同时校验最多一个可调度 leader 且必须带 fencing token。
...
graph TB
    subgraph K8s["Kubernetes Cluster"]
        ...
        subgraph NS["Namespace: tikee"]
            direction TB

            subgraph StatefulSet["tikee-server (StatefulSet, 3 replicas)"]
                S1["Pod: Server 1<br/>port 9090<br/>Leader"]
                S2["Pod: Server 2<br/>port 9090<br/>Follower"]
                S3["Pod: Server 3<br/>port 9090<br/>Follower"]
            end

            SVC["Service: tikee<br/>ClusterIP :9090"]

            subgraph CM["ConfigMap / Secrets"]
                CFG["tikee-config<br/>TOML 配置"]
                SEC["tikee-secrets<br/>DB 密码 / TLS 证书"]
            end
        ...
        subgraph App_NS["Namespace: business-apps"]
            ...
            subgraph Pod1["Pod: app-a"]
                ...
                SC1["tikee-worker<br/>sidecar"]
            end
            subgraph Pod2["Pod: app-b"]
                C2["app container<br/>:8080"]
                SC2["tikee-worker<br/>sidecar"]
            end
        end
    end

    subgraph RemoteK8s["Remote Kubernetes / Docker / VM"]
        RW["remote tikee-worker<br/>no inbound port"]
    end
...
tikee Worker 支持以 Sidecar 模式运行在 K8s Pod 中，无需修改业务应用代码：
...
spec:
  template:
    spec:
      ...
      - name: tikee-worker
        image: ghcr.io/tikee/tikee-worker:latest
        env:
        - name: TIKEE_SERVER
          value: "https://tikee.tikee.svc.cluster.local:9090"
        - name: TIKEE_APP_NAME
          value: "my-business-app"
        - name: TIKEE_PROCESSORS
...
| 形态 | 适用场景 | 网络要求 | 备注 |
|------|----------|----------|------|
| SDK 嵌入业务进程 | 业务代码直接实现处理器 | 业务进程出站访问 tikee | 延迟最低，适合核心业务任务；Java/Rust SDK 已按服务端下发 worker_id 模型接入 Worker Tunnel |
| Sidecar | 不希望业务进程直接管理调度连接 | Pod 内 localhost 调业务容器；sidecar 出站访问 tikee | 默认 K8s 推荐模式 |
| 独立 Worker Deployment | HTTP/gRPC/SQL/Script 等通用任务 | Worker 出站访问 tikee 和目标系统 | 适合共享 worker pool |
| DaemonSet | 节点级任务、文件清理、宿主观测 | 每节点 Worker 出站访问 tikee | 需更严格权限策略 |
| 跨集群 Worker Gateway | 远端集群集中接入 | gateway 出站访问中心 tikee | 可减少远端集群出口连接数 |
| 容器化脚本 Runner | 动态脚本/依赖复杂任务 | runner 出站访问 tikee | 配合 seccomp/AppArmor/NetworkPolicy |
...
本设计明确把“公共调度服务”作为部署目标，而不是只服务单个业务系统。与 xxl-job / PowerJob 相比，tikee 在 K8s、Docker 和跨集群容器部署中的关键差异如下：

| 场景 | xxl-job / PowerJob 的问题 | tikee 方案 |
|------|---------------------------|----------------|
| 同集群多 namespace | 每个 Executor/Worker 都要暴露可回连地址，NetworkPolicy 和 Service 管理复杂 | Worker 只出站连接 tikee Service，不要求业务 namespace 暴露调度端口 |
...
| 本地 Docker/Compose 调试 | external address/port 很容易配置错误 | 只需配置 `TIKEE_SERVER`，Worker 容器不声明入站 ports，不需要暴露给 Server |

因此 Helm Chart 和 Compose 模板默认不为业务 Worker 创建入站 Service/ports；只有 tikee server 暴露管理面和 Worker Tunnel 入口。
...
| 网络暴露 | Executor 必须暴露入站端口 | Worker 必须暴露入站端口，多端口多协议 | Worker 仅出站连接 tikee，减少攻击面 |
...
| 指标 | PowerJob (Java/Akka) | tikee (Rust/gRPC) | 预期提升 |
|------|---------------------|---------------------|----------|
...
tikee_server_connections_active
tikee_db_query_duration_seconds{operation}            # histogram
tikee_grpc_request_duration_seconds{method}           # histogram
```
...
│   ├── tikee-proto/
│   ├── tikee-common/
│   └── tikee-wasm/              # Worker 侧 Wasmtime 执行器；不耦合 server HTTP/storage
│
├── sdks/                             # 多语言 SDK
│   ├── rust/tikee/    # Rust Worker SDK crate
│   ├── java/tikee/           # 原生 Java SDK
│   ├── java/tikee-spring/         # Spring 集成
│   ├── java/tikee-spring-boot-starter/    # Spring Boot 集成
│   ├── go/tikee-go-sdk/           # 规划
│   ├── python/tikee-python-sdk/   # 规划
│   └── nodejs/tikee-nodejs-sdk/   # 规划
│
...
- [x] Server 集群（Raft 共识安全基础；TiKV raft-rs 已完成 bootstrap/ticker/inbound/outbound/apply/fencing/membership 与 Docker bridge smoke，生产级快照压缩/Chaos 后续增强）
  ...
  - [x] raft-rs outbound message transport skeleton（Ready messages -> HTTP wire DTO -> peer endpoint `/api/v1/raft/append-entries`；支持可选 `cluster.transport_token` / `x-tikee-raft-token`，不授予 scheduling）
  ...
  - [x] raft-rs HTTP transport token/auth/envelope smoke（本地 route harness 覆盖 `x-tikee-raft-token` 免人工 session、错误 token 拒绝、runtime inbox accepted=true 语义）
...
- [x] WASM 沙箱处理器边界（066：`WasmProcessorSpec`/`WasmResourcePolicy`/`WasmCapabilities` 稳定 worker 合约；选型 Wasmtime 45.x；默认拒绝网络与文件系统预打开）
  - [x] Worker 侧 Wasmtime 执行器基础（067：`tikee-wasm` crate；fuel/epoch interruption、memory cap、无 WASI ambient imports、策略拒绝测试、最小 WAT smoke）
...
- [x] 多语言动态脚本处理器（Python/Node/Shell/PowerShell/Rhai）
  ...
  - [x] Worker 侧语言 Runner 抽象（072：Rust SDK `ScriptRunner` / `ScriptRunnerTask` / `ScriptRunnerPolicy`，Shell/Python/Node/PowerShell/Rhai 类型识别；`script:<language>` 表示语言能力而非后端类型，默认后端规划为 WASM 通用沙箱）
  ...
  - [x] WASM 通用默认脚本沙箱（124：Java SDK/Spring Starter 启动时自动检查 Wasmtime，允许自动安装；默认注册 `script:shell` 语言能力并通过 bundled WASI shell runner 在 Wasmtime 中执行，继续默认拒绝网络、宿主文件系统和 Secret；容器 runner 作为 Docker/Podman/custom 显式覆盖后端。Python/Node/PowerShell 后续按同一模型补充 WASI runtime module 后再广告对应语言能力）
...
| Worker 身份与会话生命周期治理 | 已按 `design/worker-identity-lifecycle-design.md` 将 Worker Pool、Logical Worker Instance 和 Worker Session 分层；用 generation/fencing token 处理重启、替换、掉线、transport error 和历史归档，避免高频重启污染在线视图 | Phase 4 |
...
| 多语言动态脚本 | Python/Node/Shell/PowerShell/Rhai 等受控运行；`script:<language>` 为语言能力，默认后端为 WASM/Wasmtime，Docker/Podman/custom 为显式覆盖 | Phase 3-4 |
...
为了降低既有用户迁移成本，tikee 将同时提供 xxl-job 与 PowerJob 的迁移工具，但迁移策略不同：xxl-job 偏“补能力”，PowerJob 偏“替换架构债”。

### 16.1 从 xxl-job 迁移

可自动迁移：

1. `xxl_job_group` → tikee app / worker pool。
2. `xxl_job_info` → tikee job definition。
3. `CRON` / `FIX_RATE` → tikee schedule。
...
- Groovy decision 脚本 → tikee 安全表达式或 WASM 处理器。
...
| WASM 生态不够成熟 | 处理器灵活性 | WASM/Wasmtime 作为默认通用脚本沙箱与高安全插件方案；先以 Shell WASI runner 闭环，Python/Node/PowerShell 按 runtime module 渐进补齐；Docker/Podman/custom 作为复杂依赖场景的显式后端覆盖 |
...
tikee 的核心判断是：**xxl-job 的问题是能力不够，PowerJob 的问题是功能堆叠后架构债过重**。二者都能解决一部分“定时任务”问题，但都不适合作为企业平台中面向多团队、多语言、多集群、多租户的公共任务调度基础设施。

| 维度 | xxl-job 的思路 | PowerJob 的思路 | tikee 的思路 |
|------|----------------|-----------------|----------------------|
...
**结论**：tikee 不是对 PowerJob 或 xxl-job 的简单替代实现，而是面向企业平台的重新设计。最核心的价值不是“多几个调度类型”，而是把任务调度的通信、状态、安全、观测和多租户治理全部做成可长期演进的基础设施。
