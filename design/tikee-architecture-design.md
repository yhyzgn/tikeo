...
Job Definition
  ...
  -> Tikee 选择具备对应 runtime capability 的 Worker Pool（script:<language>/script:wasm）
...
- [ ] OIDC/SSO 集成
  ...
  - [x] OIDC discovery/JWKS 获取边界（117：callback token exchange 后读取 provider discovery、要求 `jwks_uri` 与非空 key set，但在 JWT signature/claims 验证和用户映射前仍 fail-closed 不创建 session）
- [ ] mTLS 传输加密
