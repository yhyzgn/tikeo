...
| 告警通知 | ✅ 邮件等 | ✅ 邮件/钉钉/Webhook | ✅ | Alerting 负责规则/事件/静默/升级；Notification Center 负责邮件/飞书/钉钉/企微/Slack/PagerDuty/Webhook 渠道、模板、触达、重试和 DLQ |
...
| Alerts | `GET/POST /api/v1/alert-rules`、`GET /api/v1/alert-events` | 告警规则、告警事件、静默与恢复 |
| Notifications | `GET/POST /api/v1/notification-channels`、`GET/POST /api/v1/notification-policies`、`GET /api/v1/notification-delivery-attempts` | 可复用通知渠道、模板、任务/工作流/告警订阅策略与触达记录 |
...
| Alerts | 告警规则、告警事件、静默、升级策略 | 运维告警管理；不直接承载渠道凭证 |
| Notifications | 渠道、模板、订阅策略、触达历史、重试/DLQ、测试发送 | 统一消息触达中心；供 Job、Workflow、Alert 和治理事件复用 |
...
### 9.4 Notification Center 与 Alerting 边界

告警与通知必须分层，避免把“异常规则”和“消息触达目标”混成同一个概念：

- **Alerting**：负责规则评估、异常/事故事件、severity、dedupe、silence、recovery 与 escalation 语义。现有 `alert_rules` / `alert_events` / `alert_delivery_attempts` 是告警基础，不废弃。
- **Notification Center**：负责可复用渠道、模板、订阅策略、出站投递、重试、DLQ、测试发送和触达审计。Job、Workflow、Alert、Worker 生命周期和脚本治理事件都应作为消息来源复用它。
- **Inbound Webhook Event Source**：外部系统触发 Tikeo Job 的入口，不等于出站 webhook 通知渠道，命名、API 与 UI 必须分开。

规划文档 `design/notification-center-alerting-plan.md` 固化了源代码事实、目标数据模型、API、UI、迁移阶段和验收标准。后续实现应遵循该文档：Alert Rule 不再长期内嵌 provider 凭证；任务和工作流通过 `notification_policies` 配置“哪些状态触发什么模板到哪些渠道”；告警事件也通过同一 Notification Center 触达。
...
- [x] 告警系统 (邮件/Slack/钉钉/飞书/企业微信/PagerDuty)
  ...
  - [x] Notification Center 抽象与告警边界基础（166：显式 migration/repository/entities for `notification_channels`、`notification_policies`、`notification_messages`、`notification_delivery_attempts`；新增 `/api/v1/notification-*` API、OpenAPI、RBAC/menu seed、Web `/notifications` 渠道/策略/消息/投递/DLQ UI；作业生命周期 `succeeded` / `failed` / `partial_failed` / `cancelled` / `retry_scheduled` / `retry_exhausted` / `no_eligible_worker` / `script_governance_failure` 可按 policy 物化消息；generic delivery worker 支持 webhook/Slack/钉钉/飞书/企微/PagerDuty/Email/plugin webhook-compatible provider，响应脱敏且 `secretRefs` 不回显；Alerting 与 Notification Center 边界文档化，现有 alert APIs 保持兼容）
    - [x] Notification templates CRUD/render 与 provider-specific 模板预览（167：显式 `notification_templates` migration/entity/repository；`/api/v1/notification-templates` CRUD/list/get/delete 与 `/render` 预览；安全 token replacement；Web 模板编辑/预览；policy `templateRef` 从 enabled stored templates 选择，materializer 加载 enabled 模板并写入 `payload.template`）
    - [x] Alert rule 自动迁移/dual-write 到 Notification Center policy（2026-06-17 复核：server 启动执行 `backfill_alert_rule_notification_policies`，alert rule create/update 通过 `ensure_alert_rule_notification_policy_from_channels` 写入 `notification_policies(owner_type='alert_rule')`；`alert_rules.channels_json` 保留为兼容字段，移除需等 breaking release）
    - [x] Workflow `notification` node 迁移到 registered channel/template selectors（168/169：Web 工作流通知节点使用 Notification Center channel/template selectors，不再提交 legacy raw target；后端 dry-run/create/update/validate 校验 channel 存在且启用、template 存在且启用、template provider 与渠道 provider 匹配；运行时 inline policy 物化 fail-closed，避免节点成功但无投递 attempt）
  - [x] Job 级通知绑定与消息追踪（169：`/api/v1/jobs/{job}/notification-bindings` CRUD/validate/preview 编译到 job-owned `notification_policies`；Job UI 抽屉配置 success/failure/always/advanced 触发、渠道、模板和日志选项；`/api/v1/notification-messages/{id}/trace` 汇聚 message/policy/attempt/job/instance/redacted logs，保持 Notification Center 统一投递边界）
    - [x] 异常 demo、飞书/Lark Job 卡片与公开执行透传控制台（173：各语言 demo 增加 `demo.exception` 真实运行时异常用例，SDK 将 stack trace/traceback/backtrace 写入 task log；飞书 interactive 卡片覆盖失败/成功/普通状态与“查看控制台”按钮；新增免登录 `/api/v1/public/job-instances/{id}/trace` 和 Web `/public/instances/{id}/console`，支持 `notification_delivery.public_console_base_url` 生成外部办公平台可直接打开的绝对 URL）
  - [x] Release 版本号与 lockfile 同步（169：tag 触发的 server binary / Docker server release 在 build 前运行 `scripts/set-release-version.py --scope workspace`，同步 Cargo workspace manifests、`Cargo.lock` 本地 workspace 包版本与 Helm metadata；保留 `--locked` 构建以拒绝依赖漂移，避免 `CARGO_PKG_VERSION` 停留在仓库基线版本）
    - [x] Job 级通知绑定与消息 trace（170：新增 `/api/v1/jobs/{job}/notification-bindings` CRUD/validate/preview；底层复用 job-owned `notification_policies`；支持 success/failure/always/cancelled/retry/advanced 触发预设、渠道/模板 provider 校验、Job 实例 payload 模板变量、Web Jobs 行级通知配置抽屉；新增 `/api/v1/notification-messages/{id}/trace` 与 Web 消息详情抽屉，展示 message/policy/attempts/job/instance/脱敏执行日志透传）
...
- [x] 独立文档站与 CI 验证（2026-06-10：`docs/` Docusaurus 文档站已接入主 CI `Docs site` job，运行 docs contract、Bun frozen install、typecheck 与 build；`docs/bun.lock` 使用公开 npm registry tarball URL，避免 GitHub Actions 依赖私有 npm 代理凭证；Rust/Go/Java/Python/Node SDK 页面已补 source-backed Management API create+trigger 示例，覆盖 `x-tikeo-api-key`、`triggerType=api`、默认 `executionMode=single` 与显式 broadcast selector helpers；`scripts/management-trigger-e2e-smoke.sh` 已补真实 server + Node.js demo worker + Node.js SDK ManagementClient create/trigger e2e smoke，并接入 cross-language CI smoke job 产出 artifact；docs reference 已新增 source-derived Management OpenAPI 与 Worker Tunnel protobuf 页面，中英文镜像均链接到精确 HTTP endpoint / protobuf message anchors；2026-06-10 进一步将旧 `website/` 模块迁移为 `docs/` 文档站模块，把共享媒体移至 `assets/docs/`，新增 `docs/Dockerfile` 与 `publish-docker-docs.yml` 发布 `yhyzgn/tikeo-docs`，并补齐 SEO/search/llms 与 Dashboard/Jobs/Instances/Workers/Workflows/Scripts/Audit/Settings 用户指南；随后补完 `scripts/management-trigger-e2e-smoke.sh` 贡献者 runbook 以及 Nginx Ingress/Envoy Gateway/Traefik/Gateway API 控制器专项 Kubernetes 生产 runbook，由 docs contract、真实 Management trigger e2e smoke 与 Docusaurus build 无 broken-anchor 输出保护；2026-06-10 验收加固把 docs 首页、安装、快速开始、配置参考、Rust/Go/Java/Python/Node SDK 中英文关键页升级为 operator-grade 手册，并用合同测试防止 README 复述、虚构 bootstrap 字段、不可运行 SDK 脚本和 nginx 端口映射重定向回归）
