...
| --- | --- | --- | --- | --- | --- | --- |
| B-BOOT-001 | 临时 server 启动 | 脚本生成临时 config，启动 server | `GET /readyz` 成功；端口隔离；DB 使用临时文件 | `.dev/reports/java-demo-20260601T033026Z-286798-server.log`、config | `java-demo-integration-smoke.sh` 已启动隔离 server 并通过 readyz | ✅ 通过 |
| B-AUTH-001 | 初始化管理员注册/登录 | 查询 bootstrap 状态，注册或登录 | 返回 `data.token`；bootstrap 注册后关闭 | smoke 过程 token | smoke 登录/注册成功并执行后续授权 API | ✅ 通过 |
| B-WORKER-001 | Java demo 启动 | 脚本启动 `bootRun` | `/demo/health` 成功 | `.dev/reports/java-demo-20260601T033026Z-286798-java-demo.log` | Spring demo health passed | ✅ 通过 |
| B-WORKER-002 | Worker 注册在线 | 查询 `/api/v1/workers` | `spring-demo-worker` online；namespace/app/cluster/region 正确 | `.dev/reports/java-demo-20260601T033026Z-286798-workers.json` | worker online 且 clientInstanceId= spring-demo-worker | ✅ 通过 |
| B-WORKER-003 | Worker 结构化能力 | 查询 worker capabilities/processors | processorNames、pluginProcessors、script runtime 字段结构化返回，不靠字符串约定 | workers JSON | structuredCapabilities 含 sdkProcessors 与 pluginProcessors(sql:billing.sql-sync) | ✅ 通过 |
| B-WORKER-004 | Worker master/election 字段 | 查询 `/api/v1/workers` | `master.domain/isMaster/masterWorkerId/term/fencingToken` 存在且语义正确 | workers JSON | 单 worker master 字段存在；双 worker 场景另见 D-ELECT-001 | ✅ 通过 |
| B-JOB-001 | API single 成功任务 | 创建并触发 `demo.echo` | instance `succeeded`；执行 worker 合法；日志包含 echo 业务输出 | `.dev/reports/java-demo-20260601T033026Z-286798.json` + instance/log JSON | api-single-success passed | ✅ 通过 |
| B-JOB-002 | API failure 任务 | 创建并触发 `demo.fail` | instance `failed`；错误 message/log 持久化 | report JSON、logs JSON | api-single-failure passed | ✅ 通过 |
...
| B-SCRIPT-001 | Shell 脚本任务 | 创建 shell script job 并触发 | worker 经 SRT 沙箱执行；stdout 进入 worker 控制台和实例日志 | `.dev/reports/java-demo-20260601T055026Z-640840-inst_019e81bcb36b7c63add4ca2fcc1211ae.json`、`...-logs.json`、report JSON | `script-shell-success` passed；shell=AUTO→SRT，stdout 已持久化 | ✅ 通过 |
| B-SCRIPT-002 | Python/JS/TS/Rhai 脚本任务 | 逐语言创建/触发脚本任务 | 不因 processor capability 字符串缺失卡 pending；失败时给出明确治理原因 | `.dev/reports/java-demo-20260601T055026Z-640840.json`、各 instance/logs JSON | Python/JS/TS succeeded；Rhai succeeded；无 pending | ✅ 通过 |
...
| B-QUEUE-001 | 队列无堵塞 | smoke 完成后查 queue overview | 无长期 `pending/queued` 积压；历史失效 worker 不阻塞新任务 | `.dev/reports/java-demo-20260601T055026Z-640840-dispatch-queue.json` | smoke 结束 `dispatch queue drained: done=25 failed=0` | ✅ 通过 |
...
| ID | 测试项 | 自动化步骤 | 核心断言 | 证据产物 | 当前测试结果 | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| D-BOOT-001 | 隔离 server 启动 | 临时 config：HTTP 19090、tunnel 19998、临时 DB | `/readyz` 成功 | `.dev/reports/joint-e2e-20260601T034707Z-330731-server.log`、config | joint e2e server ready | ✅ 通过 |
| D-BOOT-002 | Java demo A 启动 | 端口 18080，固定 client instance | `/demo/health` 成功 | java-a log | spring-demo-worker-a health passed | ✅ 通过 |
| D-BOOT-003 | Java demo B 启动 | 端口 18081，同 election domain | `/demo/health` 成功 | java-b log | spring-demo-worker-b health passed | ✅ 通过 |
| D-ELECT-001 | 同 domain 唯一 master | 查询 workers | 同 domain 仅 1 个 `isMaster=true` | `.dev/reports/joint-e2e-20260601T034707Z-330731-workers-before.json` | joint-worker-election passed | ✅ 通过 |
| D-DISP-001 | Single 任务优先 master | 触发 `demo.echo` single | 成功实例执行 worker 等于当前 master | instance JSON、logs JSON | joint-single-master-dispatch passed | ✅ 通过 |
...
| ID | 测试项 | 执行方式 | 核心断言 | 证据产物 | 当前测试结果 | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| F-SCRIPT-001 | Shell 脚本沙箱 | 创建/触发 shell script job | `SHELL` 默认经 SRT 沙箱执行，stdout 进 worker 控制台和实例日志 | `.dev/reports/java-demo-20260601T055026Z-640840-inst_019e81bcb36b7c63add4ca2fcc1211ae-logs.json`、demo log | `script-shell-success` passed | ✅ 通过 |
| F-SCRIPT-002 | Python 脚本治理 | 创建/触发 python script job | 若运行器不可用，明确 fail-closed；不可 pending 卡死 | `.dev/reports/java-demo-20260601T055026Z-640840-inst_019e81bcbbee73739cd1be16b5a50b03-logs.json` | Python script succeeded，stdout 入日志 | ✅ 通过 |
| F-SCRIPT-003 | JavaScript 脚本治理 | 创建/触发 JavaScript job | 运行或治理失败都有终态与日志 | `.dev/reports/java-demo-20260601T055026Z-640840-inst_019e81bcc08c7d01a6ccb7ac641e563d-logs.json` | JavaScript script succeeded，stdout 入日志 | ✅ 通过 |
| F-SCRIPT-004 | TypeScript 脚本治理 | 创建/触发 TypeScript job | 运行或治理失败都有终态与日志 | `.dev/reports/java-demo-20260601T055026Z-640840-inst_019e81bcc50f77b19c0018cf2a48139b-logs.json` | TypeScript script succeeded，stdout 入日志 | ✅ 通过 |
| F-SCRIPT-005 | Rhai 输出 | 触发 rhai script job | print 输出进入 worker 控制台与实例日志 | `.dev/reports/java-demo-20260601T055026Z-640840-inst_019e81bcc9a27353a6c9456ab91921e9-logs.json` | Rhai script succeeded，print 输出入日志 | ✅ 通过 |
| F-SANDBOX-001 | Wasmtime/SRT 自动选择 | demo 启动并触发脚本 | `shell/python/...` 原生脚本 Auto→SRT；`wasm` Auto→Wasmtime；JS/TS Auto→Deno；缺失时安装或明确失败 | demo log | 已验证 shell/python/Rhai 走 SRT 且成功，JS/TS 走 Deno 且成功；Wasmedge/V8 专项未跑 | ⏳ 待执行 |
...
2. `✅ B-SCRIPT-002 / F-SCRIPT-005` 已补齐；继续处理 `⏳ F-SANDBOX-001`：Wasmedge/V8 专项。
