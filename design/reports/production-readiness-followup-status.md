...
| Real cloud production HA validation | 🟡 | Documentation already names the required checks; release evidence script records this as opt-in rather than pretending local CI proves cloud behavior. | `docs/docs/deployment/server-ha.md`, Kind runbook, `scripts/kind-raft-ha-e2e.sh`, `scripts/verify-raft-ha-rollout.sh`, `scripts/raft-ha-fault-injection-drill.sh`. | Must still be run in the actual target environment: ingress/LB/WAF/TLS, HTTP/2 Worker Tunnel, SSE, external DB HA, multi-zone failure. |
...
2. Run Kind 4-Pod HA evidence before claiming Kubernetes semantics for a release.
...
- Kind 4-Pod HA, live Raft rollout checks, cloud ingress/LB/WAF/TLS validation, and real external DB HA were not rerun in this pass. They remain opt-in release/staging evidence gates.
