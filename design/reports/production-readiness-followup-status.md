...
| XXL-JOB / PowerJob migration tool MVP | ✅ | Moved migration out of the Server binary into dedicated `tikeo-migrate`. `tikeo-migrate plan` generates a full non-destructive bundle: Tikeo job drafts, data import split, Java/Spring project scan, Spring Boot version-aware starter recommendation, dependency/handler patch guidance, checklist, fixtures, CLI smoke tests, README/docs guidance, and source snapshots for review. `apply-data` is the explicit API-write path and supports dry-run. | `cargo test -p tikeo-migrate -- --nocapture`. | Plan is non-destructive by design: it does not connect to legacy DBs, edit source, create Tikeo Jobs, or claim broadcast/map-reduce/routing/blocking semantics are equivalent; live writes require `tikeo-migrate apply-data`. |
