...
fn container_script_runner_builds_default_deny_docker_args() {
    ...
    assert!(
        args.windows(2)
            .any(|pair| pair[0] == "--env" && pair[1] == "TIKEO_SCRIPT_ID=script_shell")
    );
    assert_eq!(
        args.iter()
            .rev()
...
async fn local_subprocess_runner_reports_unavailable_runtime() {
    let runner = LocalSubprocessScriptRunner::with_command(
        ScriptRunnerKind::Shell,
        "definitely-missing-tikeo-shell-runtime",
        ["-s".to_owned()],
    );
...
"#,
        ...
        assert!(
            home.contains(&format!("tikeo-srt-{}-runtime", kind.as_str())),
        ...
        assert!(
            !home.starts_with("/tmp/tikeo-rhai-script"),
...
"#,
        ...
        assert!(
            home.contains(&format!("tikeo-deno-{}-runtime", kind.as_str())),
            "{language} HOME should be a task runtime dir: {home}"
        );
        let runtime_root = std::path::Path::new(home)
            .parent()
