...
/**
 * Adapts {@code @SchedulerProcessor} methods to the core {@link cn.recycloud.scheduler.sdk.core.TaskProcessor} model.
 */
final class SchedulerProcessorAdapter implements SchedulerProcessorHandler {
    private final Object bean;
    private final Method method;

    private SchedulerProcessorAdapter(Object bean, Method method) {
    ...
    static SchedulerProcessorHandler forMethod(Object bean, Method method) {
        return new SchedulerProcessorAdapter(bean, method);
    }
    ...
    private static void validate(Method method) {
        if (method.getParameterCount() > 1) {
            throw new IllegalArgumentException("@SchedulerProcessor method must have zero or one parameter: " + method);
        }
        if (method.getParameterCount() == 1) {
            Class<?> parameterType = method.getParameterTypes()[0];
            boolean supported = Arrays.asList(TaskContext.class, String.class, byte[].class).contains(parameterType);
            if (!supported) {
                throw new IllegalArgumentException("unsupported @SchedulerProcessor parameter type: " + parameterType.getName());
            }
        }
        ...
        if (!supportedReturn) {
            throw new IllegalArgumentException("unsupported @SchedulerProcessor return type: " + returnType.getName());
        }
    }
}
