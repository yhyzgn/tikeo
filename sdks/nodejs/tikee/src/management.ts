export const API_KEY_HEADER = "x-tikee-api-key";
...
export class ManagementClient {
  ...
  private async send(method: string, path: string, body?: unknown): Promise<any> {
    ...
    if (!response.ok || envelope.code !== 0) throw new Error(`tikee management request failed: status=${response.status} message=${envelope.message ?? ""}`);
    if (envelope.data === undefined || envelope.data === null) throw new Error("tikee management response data was null");
