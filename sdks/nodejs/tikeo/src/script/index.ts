...
function sensitiveReadDenies(): string[] { return [".ssh", ".gnupg", ".aws", ".kube", ".docker", join(".config", "tikeo")].map((path) => join(homedir(), path)); }
function heredoc(command: string, marker: string, content: string): string { let delimiter = marker; while (content.includes(delimiter)) delimiter += "_TIKEO"; return `${command} <<'${delimiter}'\n${content}\n${delimiter}`; }
...
function addScriptEnv(env: NodeJS.ProcessEnv, task: ScriptRunnerTask): void { env.TIKEO_SCRIPT_ID = task.scriptId; env.TIKEO_SCRIPT_VERSION_ID = task.versionId; env.TIKEO_SCRIPT_VERSION_NUMBER = String(task.versionNumber); }
