...
// ScriptRunnerTask is the immutable script snapshot delivered by tikeo server.
...
func (r *ContainerScriptRunner) containerArgs(task ScriptRunnerTask) ([]string, error) {
	...
	args = append(args,
		"--env", "TIKEO_SCRIPT_ID="+task.ScriptID,
		"--env", "TIKEO_SCRIPT_VERSION_ID="+task.VersionID,
		"--env", fmt.Sprintf("TIKEO_SCRIPT_VERSION_NUMBER=%d", task.VersionNumber),
	)
