...
func (r *SrtScriptRunner) Run(ctx context.Context, task ScriptRunnerTask) (TaskOutcome, error) {
	...
	runtimeDirs, err := newScriptTaskRuntimeDirs("tikeo-srt-" + r.language + "-runtime")
	if err != nil {
		return Failed(err.Error()), nil
	}
	...
	cmd.Env = append(cmd.Env,
		"TIKEO_SCRIPT_ID="+task.ScriptID,
		"TIKEO_SCRIPT_VERSION_ID="+task.VersionID,
		fmt.Sprintf("TIKEO_SCRIPT_VERSION_NUMBER=%d", task.VersionNumber),
	)
...
func (r *DenoScriptRunner) Run(ctx context.Context, task ScriptRunnerTask) (TaskOutcome, error) {
	...
	runtimeDirs, err := newScriptTaskRuntimeDirs("tikeo-deno-" + r.language + "-runtime")
	if err != nil {
		return Failed(err.Error()), nil
	}
	defer runtimeDirs.cleanup()
...
func writeSrtSettings(task ScriptRunnerTask, runtimeDirs *scriptTaskRuntimeDirs, scriptFile string) (string, func(), error) {
	...
	file, err := os.CreateTemp("", "tikeo-srt-settings-*.json")
	if err != nil {
		return "", func() {}, err
	}
...
func sensitiveReadDenies() []string {
	...
	paths := []string{".ssh", ".gnupg", ".aws", ".kube", ".docker", filepath.Join(".config", "tikeo")}
