...
const apiKeyHeader = "x-tikee-api-key"
...
func (c *ManagementClient) send(ctx context.Context, method, path string, body any, out any) error {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 || envelope.Code != 0 {
		return fmt.Errorf("tikee management request failed: status=%s message=%s", resp.Status, envelope.Message)
	}
	if out != nil {
		if len(envelope.Data) == 0 || string(envelope.Data) == "null" {
			return fmt.Errorf("tikee management response data was null")
		}
