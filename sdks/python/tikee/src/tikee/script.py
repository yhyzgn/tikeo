...
class ContainerScriptRunner(ScriptRunner):
    ...
    def _container_args(self, task: ScriptRunnerTask) -> list[str]:
        ...
        args.extend(["--env", f"TIKEE_SCRIPT_ID={task.script_id}", "--env", f"TIKEE_SCRIPT_VERSION_ID={task.version_id}", "--env", f"TIKEE_SCRIPT_VERSION_NUMBER={task.version_number}", self.image])
...
class SrtScriptRunner(ScriptRunner):
    ...
    def run(self, task: ScriptRunnerTask) -> TaskOutcome:
        try:
            ...
            return failed(str(exc))
        runtime_dirs = ScriptTaskRuntimeDirs.create(f"tikee-srt-{self.language}-runtime")
        settings_file = None
        try:
            script_file = None
...
class DenoScriptRunner(ScriptRunner):
    ...
    def run(self, task: ScriptRunnerTask) -> TaskOutcome:
        try:
            ...
            return failed(str(exc))
        runtime_dirs = ScriptTaskRuntimeDirs.create(f"tikee-deno-{self.language}-runtime")
        try:
...
def _write_srt_settings(task: ScriptRunnerTask, runtime_dirs: ScriptTaskRuntimeDirs, script_file: Path | None) -> Path:
    ...
    fd, name = tempfile.mkstemp(prefix="tikee-srt-settings-", suffix=".json")
...
def _sensitive_read_denies() -> list[str]:
    home = Path.home()
    return [str(home / path) for path in [".ssh", ".gnupg", ".aws", ".kube", ".docker", os.path.join(".config", "tikee")]]
...
def _add_script_env(env: dict[str, str], task: ScriptRunnerTask) -> None:
    env["TIKEE_SCRIPT_ID"] = task.script_id
    env["TIKEE_SCRIPT_VERSION_ID"] = task.version_id
    env["TIKEE_SCRIPT_VERSION_NUMBER"] = str(task.version_number)
