...
API_KEY_HEADER = "x-tikee-api-key"
...
class ManagementClient:
    ...
    def _send(self, method: str, path: str, body: dict[str, Any] | None = None) -> Any:
        ...
        try:
            ...
            raise ManagementRequestError(f"tikee management response was not JSON: {response.status_code}") from exc
        if response.status_code < 200 or response.status_code >= 300 or envelope.get("code") != 0:
            raise ManagementRequestError(f"tikee management request failed: status={response.status_code} message={envelope.get('message', '')}")
        if envelope.get("data") is None:
            raise ManagementRequestError("tikee management response data was null")
