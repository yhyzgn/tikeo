...
def worker_modules():
    try:
        ...
        raise RuntimeError("grpcio-tools is required to load tikeo worker proto") from exc
    proto = Path(__file__).parent / "proto" / "worker.proto"
    out = Path.home() / ".cache" / "tikeo" / "python-proto"
    ...
    if not stamp.exists() or stamp.read_text(encoding="utf-8") != proto.read_text(encoding="utf-8"):
        ...
        if code != 0:
            raise RuntimeError(f"failed to generate tikeo worker proto: exit={code}")
