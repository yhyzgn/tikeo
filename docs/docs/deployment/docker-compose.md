...
| `/logs` | `tikeo-logs:/logs` | `tikeo-logs:/logs` | `tikeo-logs:/logs` | File logs when `observability.logging.log_dir` is `/logs`. |
...
- `observability.logging.log_dir` should stay `/logs` if you want file logs in the `tikeo-logs` volume.
