...
| `GET /api/v1/notification-messages/{id}/trace` | Inspect one message with delivery attempts, job/instance context, and redacted execution logs. | `notifications:read` plus scope check when job context resolves |
