---
title: v0.3.10 release acceptance packet
slug: release-acceptance-packet-v0.3.10
description: Evidence packet for the v0.3.10 release, including migration, HA, Notification Center, and Worker soak gates.
keywords: [tikeo v0.3.10, release acceptance, handoff, raft fsod, worker soak]
---

# v0.3.10 release acceptance packet

Use this packet when taking over development after the `v0.3.10` release. It records what was released, which evidence was collected locally, which GitHub workflows produced artifacts, and which production checks still need a real cloud environment.
...
| --- | --- |
| Release tag | `v0.3.10` |
| Release page | [v0.3.10 release](https://github.com/yhyzgn/tikeo/releases/tag/v0.3.10) |
| Release state | Published, non-draft, non-prerelease |
| Release asset count observed | `31` uploaded assets |
| Release commit evidence | `7b85b2cc fix: make migration apply in-place` |
| Latest main follow-up | `7b85b2cc fix: make migration apply in-place` |

`v0.3.10` includes the in-place migration apply flow and the release-readiness evidence scripts. The cross-language soak workflow is included as a reusable manual release-candidate gate rather than a normal CI blocker.

## Release assets observed

The `v0.3.10` GitHub Release contained these uploaded asset families at the last handoff check:

| Asset family | Observed files |
| --- | --- |
| SDK archives | `go-sdk-0.3.10.tar.gz`, `java-sdk-0.3.10.tar.gz`, `nodejs-sdk-0.3.10.tar.gz`, `python-sdk-0.3.10.tar.gz`, `rust-sdk-0.3.10.tar.gz` |
| Server binaries | `tikeo-server-0.3.10-aarch64-apple-darwin.tar.gz`, `tikeo-server-0.3.10-x86_64-apple-darwin.tar.gz`, `tikeo-server-0.3.10-x86_64-pc-windows-msvc.zip`, `tikeo-server-0.3.10-x86_64-unknown-linux-gnu.tar.gz` |
| Migration CLI binaries | `tikeo-migrate-0.3.10-aarch64-apple-darwin.tar.gz`, `tikeo-migrate-0.3.10-x86_64-apple-darwin.tar.gz`, `tikeo-migrate-0.3.10-x86_64-pc-windows-msvc.zip`, `tikeo-migrate-0.3.10-x86_64-unknown-linux-gnu.tar.gz` |
| Operator binaries | `tikeo-operator-0.3.10-darwin-amd64.tar.gz`, `tikeo-operator-0.3.10-darwin-arm64.tar.gz`, `tikeo-operator-0.3.10-linux-amd64.tar.gz`, `tikeo-operator-0.3.10-linux-arm64.tar.gz`, `tikeo-operator-0.3.10-windows-amd64.zip` |
| Terraform provider | `terraform-provider-tikeo_v0.3.10_darwin_amd64.tar.gz`, `terraform-provider-tikeo_v0.3.10_darwin_arm64.tar.gz`, `terraform-provider-tikeo_v0.3.10_linux_amd64.tar.gz`, `terraform-provider-tikeo_v0.3.10_linux_arm64.tar.gz`, `terraform-provider-tikeo_v0.3.10_windows_amd64.exe.zip` |
| Deploy and Web bundles | `tikeo-0.3.10.tgz`, `tikeo-deploy-sources-0.3.10.tar.gz`, `tikeo-web-dist-0.3.10.tar.gz`, Compose YAMLs, Kubernetes manifest, CRD manifest |

## Workflow evidence

| Workflow group | Observed result |
| --- | --- |
| Release / GitHub assets for `v0.3.10` | ✅ success |
...
| Main `CI` for `7b85b2cc` | ✅ success |
| Main `Coverage` for `7b85b2cc` | ✅ success |
| Main `CI / Migration CLI binaries` for `7b85b2cc` | ✅ success |

Before declaring a later release ready, rerun:

```bash
gh run list --branch main --limit 20
gh release view v0.3.10 --json tagName,url,isDraft,isPrerelease,assets
```
...
The source boundary included in `v0.3.10` contains a repeatable cross-language Worker soak gate in `deploy/smoke/cross-language-worker-parity-smoke.sh`. The workflow `.github/workflows/release-candidate-worker-soak.yml` exposes it as a manual release-candidate gate with configurable `ref`, `soak_seconds`, `soak_interval_seconds`, `rebuild_server`, and `skip_web` inputs. It is disabled by default in normal CI and can also be run locally:
...
Short local evidence from the soak gate run:
...
The evidence scripts are included in the `v0.3.10` source boundary and are the current way to close local handoff evidence for the three previously open areas:
...
| --- | --- | --- | --- |
| Notification Center provider/e2e | `scripts/notification-provider-e2e-smoke.sh` and `scripts/notification-real-provider-acceptance.sh` | Local `REPORT.md`, `summary.json`, provider receipt JSONL, channel test responses, message/attempt/queue snapshots; real-provider `summary.json`/`REPORT.md` or explicit `deferred_real_provider_inputs_missing` boundary | Local smoke uses a protocol-real HTTP mock provider; real scope-specific providers require saved channel ids and outbound network access. |
| Migration CLI old-project chain | `scripts/migration-cli-full-chain-smoke.sh` | `REPORT.md`, generated legacy project, in-place migrated legacy Worker project, `.tikeo-migration/`, `code-apply-evidence.json`, reviewed import payloads | Proves CLI detection, bundle generation, local code/config apply, and reviewed import payload preparation locally; domain semantic equivalence still needs a representative business project. |
...
Notification Center is ready for local/staging acceptance when provider test-send evidence exists for each active provider family in the target environment. The implementation and docs cover channel-row secrets, provider-specific template types, list/drawer test actions, retry/DLQ evidence, and redaction. `scripts/notification-provider-e2e-smoke.sh` proves the local delivery state machine end-to-end with one delivered loopback request and one forced dead-letter failure. `scripts/notification-real-provider-acceptance.sh` is the staging/production entrypoint for saved channel rows: without `TIKEO_NOTIFICATION_REAL_SERVER_URL` and `TIKEO_NOTIFICATION_REAL_CHANNEL_IDS` it archives an explicit deferred boundary instead of pretending the provider was tested. Production readiness still depends on real scope/provider calls for the deployment environment.
...
| P2 before broad migration promotion | Exercise `tikeo-migrate` against a representative legacy XXL-JOB or PowerJob project. Local rehearsal can be produced with `scripts/migration-cli-full-chain-smoke.sh`. | Local in-place apply evidence plus at least one staging trigger with behavior comparison. |
