...
- 如需更强隔离，设置 `TIKEO_SANDBOX_REQUIRE_MANAGED_TOOLS=1`（Java Boot：`tikeo.worker.scripts.require-managed-tools=true`）。开启后 SDK 会忽略宿主 `PATH` 工具和解释器，只使用 `TIKEO_SANDBOX_TOOLS_DIR` / `~/.tikeo/sandbox-tools` 中的托管二进制，未就绪时 fail-closed。
