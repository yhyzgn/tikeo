...
| Processor name | `invoice.send-reminder` | 两者 | Job binding 必须匹配 Worker 广告的 SDK processor。 |
...
| SDK processors | `add_sdk_processor` | `AddSDKProcessor` | `@TikeoProcessor` | `add_sdk_processor` | `addSDKProcessor` | 广告 processor names。 |
...
| SDK processors | Scheduler 按 processor name 路由 SDK jobs。 | 只添加已实现 processors。 |
