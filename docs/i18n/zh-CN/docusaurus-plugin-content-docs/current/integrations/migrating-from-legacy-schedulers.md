...
`plan` 刻意设计为非破坏性命令。它 **不会** 修改旧源码、不会创建 Tikeo Job。它可能读取旧调度器数据库来生成复核包。`apply` 是本地命令，但会直接修改旧 Java Worker 项目：改依赖、改 processor、把 Tikeo 配置占位符写到原来已经配置 XXL-JOB 或 PowerJob 的 Spring 配置文件中。它不会调用 Tikeo Server。
...
`plan` 只读。要执行机械性的 Java 代码迁移，请在旧 Worker 项目的迁移分支中运行 `apply`。这个命令会有意地直接修改当前旧项目：
...
本地 `apply` 之后，在迁移后的项目中用选定 SDK 版本编译，例如：
...
执行 `apply` 后，打开项目中被修改的配置文件，例如 `src/main/resources/application.yml`、`application-dev.yml`、`application-prod.yml`、`bootstrap.yml` 或 `bootstrap-prod.yml`。CLI 的选择顺序是：
...
3. 如果没有任何 Spring 配置文件，则只在迁移项目中新建 `application.yml`。
