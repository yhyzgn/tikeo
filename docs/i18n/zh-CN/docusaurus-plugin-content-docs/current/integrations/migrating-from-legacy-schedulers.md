...
| `code-apply-evidence.json` | `apply-code` 生成；记录源项目、输出项目、变更文件、跳过的生成/构建目录和 warnings。 | 和 CI/PR 证据一起保留。 |
| `CODE_MIGRATION_REPORT.md` | `apply-code` 生成；同时写到迁移包和迁移后的输出项目。 | 作为代码评审和交接材料。 |
...
`plan` 只读。要执行机械性的 Java 代码迁移，请使用 `apply-code`，并把输出目录放在原项目之外：
...
- 写入 `src/main/resources/application-tikeo-migration.yml`，包含待复核的 Tikeo Worker 配置，并关闭旧 Worker flags；
...
只有明确要覆盖已有输出副本时才使用 `--force`。原 Worker 项目不会被修改。`apply-code` 之后，在迁移副本中用选定 SDK 版本编译，例如：
