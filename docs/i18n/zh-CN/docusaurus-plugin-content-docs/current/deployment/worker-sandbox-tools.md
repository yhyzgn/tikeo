...
| 严格沙箱隔离 | `TIKEO_SANDBOX_STRICT_ISOLATION=1`；Java Boot 也可用 `tikeo.worker.scripts.strict-sandbox-isolation=true` | SDK 跳过宿主 `PATH` 工具和解释器，只使用 `TIKEO_SANDBOX_TOOLS_DIR` / `~/.tikeo/sandbox-tools` 下的二进制。缺失工具不会声明能力，并 fail-closed。 | 多租户、强合规、变更受控 Worker。 |
