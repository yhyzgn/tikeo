...
| 6 | 部署页面 | 单二进制、Docker Compose、Kubernetes/Helm、[Server 高可用与集群模式](./deployment/server-ha)、控制器差异和 smoke 脚本。 |
