...
| 跨语言 Worker soak | 可选的 release-candidate 运行时门禁，用于验证 Go/Rust/Python/Node 多轮派发、任务日志和 queue/outbox metrics。 | [SDK 与 API 集成](../integrations/sdk-and-api)、[Worker 用户指南](../user-guide/workers) | `TIKEO_CROSS_SOAK_SECONDS=120 deploy/smoke/cross-language-worker-parity-smoke.sh`、`*-soak-summary.json`、`*-soak-summary.csv`、`*-soak-metrics.jsonl`。 |
...
| Dry-run apply | 执行 `tikeo-migrate apply --endpoint <staging> --api-key <key> --dry-run`。 | dry-run 输出，包含计划创建/更新项和被拒绝 job。 |
...
| 迁移 CLI | 示例 `.tikeo-migration` bundle、dry-run apply 输出、release assets。 |
