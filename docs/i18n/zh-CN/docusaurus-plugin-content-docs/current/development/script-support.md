...
Tikeo 的脚本支持面向小型运维执行器：它们需要审查、版本化、回滚、stdout/stderr 捕获和 Job 绑定，但不一定值得每次都发布一个编译型 Normal Processor。脚本不是藏在 Server 里的 shell escape。Server 保存元数据、版本、RBAC、审计和绑定关系；Worker 通过允许的运行时执行已发布版本，并按普通实例证据回传结果。
...
当逻辑较短、偏运维、需要快速审查时使用脚本，例如清理、健康探测、数据导出交接或简单集成调用。逻辑庞大、性能敏感、依赖复杂或需要按应用代码测试发布时，应优先使用 Normal Processor。
