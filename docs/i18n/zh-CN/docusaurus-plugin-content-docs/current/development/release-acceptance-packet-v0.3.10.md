...
description: v0.3.10 发布交接证据包，包含迁移、HA、通知中心和 Worker soak 门禁。
keywords: [tikeo v0.3.10, 发布验收, 交接, raft fsod, worker soak]
---

# v0.3.10 发布验收包

接手 `v0.3.10` 之后的开发时使用本页。本页记录已经发布的内容、本地保留的证据、GitHub workflow 产物，以及仍需要真实云环境才能完成的生产检查。
...
| --- | --- |
| Release tag | `v0.3.10` |
| Release 页面 | [v0.3.10 release](https://github.com/yhyzgn/tikeo/releases/tag/v0.3.10) |
| Release 状态 | 已发布，非 draft，非 prerelease |
| 已观察到的资产数量 | `31` 个 uploaded assets |
| Release 提交证据 | `7b85b2cc fix: make migration apply in-place` |
| 最新 main 提交 | `7b85b2cc fix: make migration apply in-place` |

`v0.3.10` 已包含原地迁移 apply 流程（in-place migration apply）和 release-readiness 证据脚本。跨语言 soak workflow 作为可复用的手动 release-candidate gate 随源码边界保留，不作为普通 CI 阻塞项。

## 已观察到的 Release 资产

最后一次交接检查时，`v0.3.10` GitHub Release 包含以下已上传资产族：

| 资产族 | 已观察文件 |
| --- | --- |
| SDK 压缩包 | `go-sdk-0.3.10.tar.gz`、`java-sdk-0.3.10.tar.gz`、`nodejs-sdk-0.3.10.tar.gz`、`python-sdk-0.3.10.tar.gz`、`rust-sdk-0.3.10.tar.gz` |
| Server 二进制 | `tikeo-server-0.3.10-aarch64-apple-darwin.tar.gz`、`tikeo-server-0.3.10-x86_64-apple-darwin.tar.gz`、`tikeo-server-0.3.10-x86_64-pc-windows-msvc.zip`、`tikeo-server-0.3.10-x86_64-unknown-linux-gnu.tar.gz` |
| 迁移 CLI 二进制 | `tikeo-migrate-0.3.10-aarch64-apple-darwin.tar.gz`、`tikeo-migrate-0.3.10-x86_64-apple-darwin.tar.gz`、`tikeo-migrate-0.3.10-x86_64-pc-windows-msvc.zip`、`tikeo-migrate-0.3.10-x86_64-unknown-linux-gnu.tar.gz` |
| Operator 二进制 | `tikeo-operator-0.3.10-darwin-amd64.tar.gz`、`tikeo-operator-0.3.10-darwin-arm64.tar.gz`、`tikeo-operator-0.3.10-linux-amd64.tar.gz`、`tikeo-operator-0.3.10-linux-arm64.tar.gz`、`tikeo-operator-0.3.10-windows-amd64.zip` |
...
| 部署与 Web 包 | `tikeo-0.3.10.tgz`、`tikeo-deploy-sources-0.3.10.tar.gz`、`tikeo-web-dist-0.3.10.tar.gz`、Compose YAML、Kubernetes manifest、CRD manifest |
...
`v0.3.10` 源码边界包含 `deploy/smoke/cross-language-worker-parity-smoke.sh` 中可重复执行的跨语言 Worker soak gate。workflow `.github/workflows/release-candidate-worker-soak.yml` 把它暴露为手动 release-candidate gate，并支持 `ref`、`soak_seconds`、`soak_interval_seconds`、`rebuild_server`、`skip_web` 输入。普通 CI 默认不启用，也可以在本地显式运行：
...
以下证据脚本已包含在 `v0.3.10` 源码边界中；当前可用以下命令一次性收集三个遗留项的本地交接证据：
