...
- RBAC 当前是最小 `resource/action` 模型；API Token 已有 create/list/revoke、细粒度 `resource:action` scope、TTL 策略和 rotate 基础，但尚未实现 namespace/app/worker_pool scope、多租户隔离或完整 OIDC/SSO。
