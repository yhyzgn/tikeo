...
- RBAC 当前是最小 `resource/action` 模型；API Token 已有 create/list/revoke、细粒度 `resource:action` scope、TTL 策略、rotate 与 namespace/app/worker_pool binding 基础；tenant/app/worker-pool 后端管理 API 已具备 create/list 基础，完整 UI、删除/级联策略、租户隔离策略闭环和 OIDC 身份映射仍未实现。
