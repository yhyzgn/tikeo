...
7. Coverage 已新增 Rust workspace 覆盖率 workflow：`.github/workflows/coverage.yml` 使用 `cargo llvm-cov` + `codecov/codecov-action@v5` + `CODECOV_TOKEN` 上传 `rust` flag；README 已加入 Codecov Rust coverage badge。若要项目总覆盖率，后续再补 Web/Java/Go/Python/Node coverage 并明确合并口径。
