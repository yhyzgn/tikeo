...
7. Coverage 已新增并验证：`.github/workflows/coverage.yml` 使用 `cargo llvm-cov` + `codecov-cli` + `CODECOV_TOKEN` 上传 Rust workspace `rust` flag；run `27121393205` 成功，README Codecov Rust coverage badge 已返回 84%。若要项目总覆盖率，后续再补 Web/Java/Go/Python/Node coverage 并明确合并口径。
