...
1. **先读上下文，再开发**：开始任何任务前必须阅读：
   - `./prompt.md`（本文件）
   - `./design/tikeo-architecture-design.md`（架构与产品设计源）
   - `./.memory/README.md`
...
3. **路线图完成项必须回写设计文档**：每个开发工作项完成后，必须在 `./design/tikeo-architecture-design.md` 的开发路线图中把对应条目标记为完成 `[x]`，不额外添加 ✅ 图标。
...
7. **不丢设计目标**：实现必须服从 `./design/tikeo-architecture-design.md`，若代码实现需要偏离设计，必须先更新设计文档和 `./.memory/decisions.md`。
...
10. **不要让 Worker 暴露入站端口**：tikeo 的核心架构是 Worker 主动通过 gRPC/HTTP2 tunnel 连接 Server，Server 反向指令复用该长连接。
...
`tikeo` 是一个用 Rust 从零构建的分布式任务调度与计算平台，目标是成为企业内部公共任务调度基础设施。
...
详细设计以 `./design/tikeo-architecture-design.md` 为准。
...
```text
🚀 建立 tikeo 调度核心的可验证基础骨架

Context:
- 基于 design/tikeo-architecture-design.md 的 Phase 1 目标推进
...
```text
Constraint: design/tikeo-architecture-design.md Phase 1
Confidence: high
Scope-risk: narrow
...
4. **004-storage-and-tikeo**：SeaORM、SQLite/MySQL、Job/Instance 模型、CRON/FIX_RATE。
...
cat ./prompt.md
cat ./design/tikeo-architecture-design.md
