# Java Spring Boot 4.x Worker Demo

Standard Spring Boot Web demo for `sdks/java/tikeo-spring-boot-starter`, validating the compatibility starter in a Spring Boot 4.x application.

Build, test, and run independently from the repository root. Use the committed Gradle Wrapper; the demo targets Java 17 bytecode and runs on Spring Boot 4.x to validate the matching tikeo starter artifact.

```bash
(cd examples/java/spring-boot4-worker-demo && ./gradlew test)
(cd examples/java/spring-boot4-worker-demo && ./gradlew bootRun)
```
...
- `SpringBootStarterCompatibilityMatrixTest` — verifies this demo uses `tikeo-spring-boot-starter` and that the Java SDK exposes real Boot 2, Boot 3, and Boot 4 starter modules with concrete `src` trees.

The demo is a normal embedded-web Spring Boot application. `bootRun` stays online through the web server, not a custom blocking runner. It exposes `GET /demo/health` and `GET /demo/processors` on `TIKEO_DEMO_SERVER_PORT` (default `18084`).
...
```bash
(cd examples/java/spring-boot4-worker-demo && TIKEO_WORKER_ENDPOINT=http://127.0.0.1:9998 TIKEO_DEMO_SERVER_PORT=18084 ./gradlew bootRun)
```
...
This demo is the Spring Boot 4.x example and therefore depends on `tikeo-spring-boot-starter`.
...
```bash
(cd examples/java/spring-boot4-worker-demo &&   TIKEO_MANAGEMENT_ENABLED=true   TIKEO_MANAGEMENT_ENDPOINT=http://127.0.0.1:9999   TIKEO_MANAGEMENT_API_KEY=<tk-api-key>   ./gradlew bootRun)
```
