...
| Demo | Spring Boot | tikeo starter | Default port | Verification |
| --- | --- | --- | --- | --- |
| `spring-boot2-worker-demo` | 2.7.x | `tikeo-spring-boot2-starter` | `18082` | `(cd examples/java/spring-boot2-worker-demo && ./gradlew clean test --no-daemon)` |
| `spring-boot3-worker-demo` | 3.5.x | `tikeo-spring-boot3-starter` | `18083` | `(cd examples/java/spring-boot3-worker-demo && ./gradlew clean test --no-daemon)` |
| `spring-boot4-worker-demo` | 4.0.x | `tikeo-spring-boot-starter` | `18084` | `(cd examples/java/spring-boot4-worker-demo && ./gradlew clean test --no-daemon)` |
...
Use the development integration seed and worker matrix scripts when validating one tikeo server with multiple Java demo workers in different tenant scopes.
...
The demo applications read `TIKEO_WORKER_NAMESPACE`, `TIKEO_WORKER_APP`, and `TIKEO_WORKER_POOL` from the environment. `worker_pool` is also advertised as a worker label so the server worker list can bind each worker to the seeded pool.
