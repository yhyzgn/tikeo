      ...
      <section className="login-page__shell setup-page__shell" aria-label={t('tikeo 初始化入口')}>
        <div className="login-page__visual setup-page__visual">
          ...
          <Typography.Paragraph className="login-page__summary">
            {t('创建站点第一个 Owner 账号，完成后注册入口会自动关闭，后续成员统一由 Owner 或授权管理员邀请和维护')}
        ...
        <Card className="login-page__card login-card setup-card">
          ...
          <Typography.Title className="login-page__form-title" level={2}>{t('创建 Owner')}</Typography.Title>
          <Typography.Paragraph className="login-page__form-copy" type="secondary">
            {t('请设置可长期维护站点的 Owner 身份')}
          </Typography.Paragraph>
          ...
          >
            <Form.Item name="username" label={t('用户名')} rules={[{ required: true, message: t('请输入用户名') }]}>
              <Input prefix={<UserOutlined />} autoComplete="username" placeholder="owner" />
            </Form.Item>
            <Form.Item
              name="email"
              label={t('邮箱')}
