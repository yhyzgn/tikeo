# 010-tikeo-tick-loop：CRON / Fixed Rate 自动触发
...
- `tikeo-storage` 已支持 Job/JobInstance 持久化、pending 查询、status update。
- `tikeo-server::tunnel::dispatcher` 已定期分发 pending instance 到 first available Worker。
- `tikeo::WorkerSession::process_next` 已能接收 dispatch 并回传结果。
