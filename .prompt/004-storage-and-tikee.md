# 004-storage-and-tikee：存储层与基础调度器
...
- Rust 模块位于 `./crates/*`。
- 已有 `tikee-proto`，包含 Worker Tunnel proto/gRPC bindings。
- `tikee-server` 已启动 HTTP `9090` 与 Worker Tunnel gRPC `9998`。
...
- 存储层应独立 crate，例如 `crates/tikee-storage`。
- 依赖默认使用当前最新稳定版；不能使用最新版必须记录原因。
- 完成路线图项目后，必须在 `design/tikee-architecture-design.md` 开发路线图中使用 `[x]` 标记，不额外添加 ✅ 图标。

## 建议任务

1. 新增 `crates/tikee-storage`。
