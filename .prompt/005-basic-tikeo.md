# 005-basic-tikeo：基础调度器与 API 触发链路
...
- `crates/tikeo-storage` 已提供 namespace/app/job/job_instance schema、migration 与 `JobRepository`。
...
1. 在 `tikeo-core` 或新增独立 crate 中定义调度领域模型：ScheduleType、TriggerType、InstanceStatus、DispatchDecision。
2. 扩展 `tikeo-storage`：补充 job_instance repository，支持创建 API 触发实例、查询实例列表/详情。
3. 扩展 HTTP API：
