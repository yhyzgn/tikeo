...
- The job runs:
  ...
  - `bun run docs:build`
- `docs/bun.lock` tarball URLs now use public `https://registry.npmjs.org/` instead of the previous private Nexus registry host.
...
- Do not let `docs/bun.lock` regress to private registry tarball URLs.
