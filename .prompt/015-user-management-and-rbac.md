...
- 确认 `crates/scheduler-storage` 引入 Users 相关 Entity 并生成 migration。
- 确认 `crates/scheduler-server/src/http/auth.rs` 能够解析真实 Token 并检查 DB 或内存缓存以提供完整的 Authentication 和 Authorization。
