# 037 — Java SchedulerProcessor method adapter

## Context
Java Core SDK now has a real `GrpcSchedulerWorkerClient` and generated Worker Tunnel protobuf bindings. Spring Boot auto-configuration creates the real client by default, with `scheduler.worker.dry-run=true` available for demos/tests.
...
## Next goal
Make Spring Boot `@SchedulerProcessor` annotations execute real dispatched tasks instead of the current default success processor.

## Required work
1. Extend `SchedulerProcessorRegistry` to store invocable method metadata, not only bean references.
