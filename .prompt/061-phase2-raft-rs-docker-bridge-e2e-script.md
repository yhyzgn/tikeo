...
- Correct `x-tikeo-raft-token` can bypass human session auth for internal transport only; wrong token is rejected via normal auth envelope.
...
## Required next work
1. Add a developer E2E script (prefer `scripts/raft-bridge-e2e.sh`) that starts multiple tikeo containers/services on Docker bridge networking, not host mode.
2. Use service/container DNS or explicit bridge IPs for `cluster.peers[*].endpoint`; keep `0.0.0.0` bind and `9998` port defaults.
3. Inject `TIKEO__CLUSTER__TRANSPORT_TOKEN` via environment/secret-like local variable; do not commit production secrets.
