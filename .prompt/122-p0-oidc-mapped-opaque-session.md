...
## Goal
Complete the P0 OIDC path so an external `(issuer, subject)` identity can be mapped to a local tikeo user/role/scope and receive a local opaque tikeo session.
...
- JWT/id_token/provider access token must not become tikeo local session state.
...
## Verification
- `rtk cargo test -p tikeo-server oidc_callback_issues_opaque_session_for_mapped_external_subject --all-features`
- `rtk cargo test -p tikeo-server oidc --all-features`
- `rtk cargo clippy -p tikeo-storage --all-targets --all-features -- -D warnings`
- `rtk cargo clippy -p tikeo-server --all-targets --all-features -- -D warnings`
