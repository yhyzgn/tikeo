...
- outbound peer HTTP transport skeleton with optional `cluster.transport_token` / `x-tikeo-raft-token`;
...
## Hard safety rule
Do not weaken the no-fake-leader guarantee. Raft mode may schedule only when status has `can_schedule=true` **and** a persisted `leader_fencing_token`; tikeo/dispatcher ownership gates must continue to consume that boundary.
