...
- Spring `@SchedulerProcessor` methods are invocable through `SpringSchedulerTaskProcessor`.
...
4. Change Java `SpringSchedulerTaskProcessor` routing from `jobId()` fallback to explicit processor name, with temporary fallback only for compatibility.
