...
## Scope
- Add `observability/prometheus/tikeo-recording-rules.yml` for core HTTP, dispatch queue, workflow, worker, and script-governance SLO series.
- Add a minimal Prometheus config that scrapes tikeo `/metrics` and loads the rules.
...
## Validation target
- `cargo test -p tikeo-server grafana --all-features`
- `cargo check -p tikeo-server --all-features`
