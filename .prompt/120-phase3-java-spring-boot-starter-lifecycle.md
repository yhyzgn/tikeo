...
## Completed scope
- Added `TikeoWorkerLifecycle` as a Spring `SmartLifecycle` bridge around `TikeoWorkerClient`.
- `tikeo.worker.enabled=false` disables worker client/lifecycle beans while leaving processor registry scanning available.
- `tikeo.worker.auto-startup=false` keeps the client bean available but prevents automatic startup.
...
## Verification
- `rtk bash -lc 'cd sdks/java && ./gradlew :tikeo-spring-boot-starter:test --warning-mode all --no-daemon'`
