...
## Completed scope
- Added `tikee_server::observability::tracing::TracingRuntime` as a focused runtime module instead of embedding exporter setup in route handlers.
...
## Verification
- `rtk cargo test -p tikee-server --test otel_exporter_smoke --all-features`
- `rtk cargo test -p tikee-server observability_status_reports_default_and_configured_otlp_without_collector --all-features`
- Targeted fmt/clippy for `tikee-server`.
