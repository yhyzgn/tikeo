...
在后端、Worker SDK、Java Starter 骨架和 Web 管理端基础工程已完成后，补齐容器化部署基础，确保 tikee server 和未来 worker 可在不同容器、namespace 或集群中部署。

## 当前上下文

- 后端根 binary：`src/main.rs`，命令 `tikee serve --config config/dev.toml`。
...
- SQLite dev DB URL：`sqlite://tikee-dev.db?mode=rwc`。

## 建议任务

1. 新增后端多阶段 `Dockerfile`，构建 Rust release binary。
2. 新增 Web Docker build 或将 Web 静态资源作为独立 nginx/caddy 容器服务。
3. 新增 `docker-compose.yml`，至少包含 tikee server；可选包含 web 服务。
