...
- `EntryNormal` payloads are parsed as a scheduler command envelope: `{ "command_id": string, "command_type": string, "payload": object }`.
...
## Required next work
1. Choose the first real business command that belongs in Raft (e.g. scheduler ownership metadata, cluster membership proposal metadata, or dispatch shard lease state) and document why it is safe.
