# 010-tikee-tick-loop：CRON / Fixed Rate 自动触发
...
- `tikee-storage` 已支持 Job/JobInstance 持久化、pending 查询、status update。
- `tikee-server::tunnel::dispatcher` 已定期分发 pending instance 到 first available Worker。
- `tikee::WorkerSession::process_next` 已能接收 dispatch 并回传结果。
