<p align="center">
  <img src="web/src/assets/tikeo-logo.svg" alt="tikeo logo" width="112" height="112" />
</p>

<p align="center"><strong>tikeo · Distributed Task Orchestration Platform</strong></p>

# tikeo

`tikeo` 是一个 Rust workspace 模式开发的分布式任务调度平台。后端主入口在根目录 `src/main.rs`，核心模块拆分在 `crates/*`；Web 管理端在 `web/`，使用 React + Ant Design + Bun。
...
初始化注册入口只在用户表为空时开放；创建首个管理员后立即关闭。后续管理员、操作员、查看者等账号只能由站内管理员在“用户管理”中手动创建。所有受保护 API 都必须先通过登录接口获取 48 位 opaque base62 会话 token。
...
```bash
./scripts/dev-seed.sh              # 默认写入 tikeo-dev.db
...
- `dev_operator` / `dev_viewer` 两个开发账号，密码为 `Tikeo@2026!`；它们不会替代首次初始化管理员。
...
| 数据库 | URL 示例 | 说明 |
| --- | --- | --- |
| SQLite | `sqlite://tikeo-dev.db?mode=rwc` | 本地开发默认 |
| MySQL | `mysql://user:pass@mysql:3306/tikeo` | 生产可用 |
| PostgreSQL | `postgres://user:pass@postgres:5432/tikeo` | Phase2 已启用 sqlx-postgres |
| CockroachDB | `postgres://root@cockroach:26257/tikeo?sslmode=disable` | 复用 PostgreSQL wire protocol |

环境变量覆盖示例：

```bash
export TIKEO__STORAGE__DATABASE_URL="postgres://tikeo:tikeo@postgres:5432/tikeo"
./target/debug/tikeo serve --config config/postgres.toml
```
...
- SDK 总目录：`sdks/<language>/<sdk-name>/`，例如 `sdks/rust/tikeo/`、`sdks/java/tikeo-spring-boot-starter/`。
...
- 根 `Dockerfile` 只构建 tikeo 服务端镜像，绝不复制、缓存或构建 `sdks/` 与 `examples/`。
...
(cd sdks/java && ./gradlew test)
(cd sdks/java && ./gradlew :tikeo-spring-boot-starter:test)
(cd examples/java/spring-boot3-worker-demo && ./gradlew test)
(cd examples/java/spring-boot3-worker-demo && TIKEO_WORKER_DRY_RUN=false TIKEO_WORKER_ENDPOINT=http://127.0.0.1:9998 ./gradlew bootRun)
```
...
```bash
DOCKER_BUILDKIT=1 docker build -t tikeo/server:dev .
